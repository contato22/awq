-- ═══════════════════════════════════════════════════════════════════════════════
-- AWQ — Supabase Schema (all tables except AWQ CRM, which is in supabase_crm_setup.sql)
...
create table if not exists jacqes_crm_interactions (
  ...
  satisfacao_percebida  text not null default '',
  risco_percebido       text not null default '',
  created_at            timestamptz not null default now()
);
...
create table if not exists ma_deals (
  ...
  traction_score              numeric not null default 0,
  total_score                 numeric not null default 0,
  screening_decision          text,
  screening_notes             text,
  dd_status                   text,
  dd_completion_pct           numeric not null default 0,
  dd_start_date               text,
  dd_end_date                 text,
  proposed_valuation          numeric,
  proposed_investment_amount  numeric,
  proposed_equity_pct         numeric,
  media_commitment_value      numeric,
  media_delivery_period_months integer,
  board_seat                  boolean not null default false,
  observer_rights             boolean not null default false,
  vesting_period_years        integer,
  vesting_cliff_months        integer,
  ic_memo_url                 text,
  ic_presentation_url         text,
  ic_meeting_date             text,
  ic_decision                 text,
  ic_decision_date            text,
...
create table if not exists ma_portfolio_companies (
  ...
  portco_code                 text not null default '',
  legal_name                  text not null default '',
  trade_name                  text,
  document_number             text,
  deal_id                     text,
  deal_type                   text not null default 'm4e',
  investment_date             text not null default '',
  awq_ownership_pct           numeric,
  awq_shares_held             numeric,
  total_shares_outstanding    numeric,
  entry_valuation             numeric,
  current_valuation           numeric,
  valuation_date              text,
  media_commitment_value      numeric,
  media_delivered_value       numeric not null default 0,
  media_remaining_value       numeric,
  media_delivery_start_date   text,
  media_delivery_end_date     text,
  board_seat                  boolean not null default false,
  observer_rights             boolean not null default false,
...
create table if not exists ma_due_diligence_items (
  ...
  dd_category       text not null default 'financial',
...
create table if not exists ma_portco_kpis (
  ...
  funding_round_closed  boolean not null default false,
  notes                 text,
  submitted_by          text,
  created_at            timestamptz not null default now(),
  updated_at            timestamptz not null default now()
...
create table if not exists ma_board_meetings (
  ...
  meeting_type            text not null default 'monthly_review',
  agenda                  text,
  board_deck_url          text,
...
create table if not exists ma_media_deliverables (
  ...
  approved_by_portco        boolean not null default false,
  approval_date             text,
  approval_notes            text,
...
create table if not exists ma_ic_meetings (
  ...
  meeting_date   text not null default '',
  meeting_type   text not null default 'deal_review',
  attendees      jsonb,
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- Row Level Security — allow anon + authenticated for all tables
-- ═══════════════════════════════════════════════════════════════════════════════
...
declare
  ...
  tables text[] := array[
    ...
    'ppm_projects','ppm_tasks','ppm_milestones','ppm_allocations',
    'ppm_time_entries','ppm_risks','ppm_issues'
  ];
begin
  foreach tbl in array tables loop
    execute format('alter table %I enable row level security', tbl);
    execute format(
      'create policy if not exists allow_all on %I for all to anon, authenticated using (true) with check (true)',
      tbl
    );
  end loop;
end $$;
