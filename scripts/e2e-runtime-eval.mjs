...
try {
  ...
  if (kpiCount === 8)
    ...
    warn(`caza-stats.json — expected 8 KPIs, got ${kpiCount} (run fetch-notion-static.mjs to populate)`);
} catch(e) {
  // caza-stats.json is generated at runtime by fetch-notion-static.mjs — not committed to repo
  warn("caza-stats.json — not found (generated by Notion fetch; not required for SSR CI build)");
  stats = null;
}
...
if (stats) {
  ...
  if (missingKpis.length === 0)
    ...
    warn(`caza-stats.json — missing KPI IDs: ${missingKpis.join(", ")}`);
...
if (branch && branch !== "main" && branch !== "master")
  ...
  warn(`On main branch: ${branch} — expected a feature branch`);
else
