...
export default function AccountsPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const localAccounts: CrmAccount[] = JSON.parse(localStorage.getItem("awq_local_accounts") ?? "[]");
      try {
        const res = await fetch("/api/crm/accounts");
        const json = await res.json();
        const base: CrmAccount[] = json.success ? json.data : [];
        const merged = [...localAccounts, ...base.filter(a => !localAccounts.some(l => l.account_id === a.account_id))];
        setAllAccounts(applyLocalState(merged));
      } catch {
        setAllAccounts(applyLocalState(localAccounts));
      } finally {
        setLoading(false);
      }
    }
  ...
  const accounts = useMemo(() => {
    return allAccounts.filter(a => {
      if (filterType !== "Todos" && a.account_type !== filterType) return false;
      if (filterBu !== "Todos" && a.bu !== filterBu) return false;
      if (filterOwner !== "Todos" && a.owner !== filterOwner) return false;
      if (search) {
        const q = search.toLowerCase();
        if (!a.account_name.toLowerCase().includes(q) && !(a.trade_name ?? "").toLowerCase().includes(q)) return false;
      }
      return true;
    });
  }, [allAccounts, filterType, filterBu, filterOwner, search]);
