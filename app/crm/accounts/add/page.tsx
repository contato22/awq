...
import {
  ...
  Building2, MapPin, Users, ChevronLeft, Calendar, Mail, Phone,
} from "lucide-react";
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="mt-4 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {importedData.phone && (
                      <div className="flex items-center gap-1.5 text-[11px] text-gray-600">
                        ...
                        <span>{importedData.phone}{importedData.phone2 ? ` / ${importedData.phone2}` : ""}</span>
                      </div>
                    )}
                    {importedData.email && (
                      <div className="flex items-center gap-1.5 text-[11px] text-gray-600">
                        <Mail size={10} className="text-gray-400 shrink-0" />
                        <span className="truncate">{importedData.email}</span>
                      </div>
                    )}
                    {importedData.data_abertura && (
                      <div className="flex items-center gap-1.5 text-[11px] text-gray-600">
                        <Calendar size={10} className="text-gray-400 shrink-0" />
                        <span>Abertura: {importedData.data_abertura}</span>
                      </div>
                    )}
                    {importedData.natureza_juridica && (
                      <div className="flex items-center gap-1.5 text-[11px] text-gray-600">
                        <Building2 size={10} className="text-gray-400 shrink-0" />
                        <span className="truncate">{importedData.natureza_juridica}</span>
                      </div>
                    )}
                    {importedData.opcao_pelo_simples && (
                      <div className="flex items-center gap-1.5 text-[11px] text-emerald-700">
                        <CheckCircle2 size={10} className="shrink-0" />
                        <span>Simples Nacional</span>
                      </div>
                    )}
                    {importedData.opcao_pelo_mei && (
                  ...
                  {importedData.cnae_descricao && (
                    <p className="text-[11px] text-gray-500">
                      <span className="font-medium text-gray-700">CNAE principal:</span> {importedData.cnae_descricao}
                    </p>
                  )}
                  {importedData.cnaes_secundarios.length > 0 && (
                    <div className="text-[11px] text-gray-500">
                      <span className="font-medium text-gray-700">CNAEs secundários: </span>
                      {importedData.cnaes_secundarios.map(c => c.descricao).join(" · ")}
                    </div>
                  )}
                  {/* Capital social */}
                  {importedData.capital_social != null && importedData.capital_social > 0 && (
                    <p className="text-[11px] text-gray-500">
                      <span className="font-medium text-gray-700">Capital social:</span>{" "}
                      R$ {importedData.capital_social.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}
                    </p>
                  )}
                  {/* Sócios */}
                  {importedData.socios.length > 0 && (
                    <div>
                      <p className="text-[11px] font-medium text-gray-700 mb-1.5">Quadro societário</p>
                      <div className="flex flex-wrap gap-1.5">
                        {importedData.socios.map((s, i) => (
                          <div key={i} className="flex items-center gap-1.5 px-2 py-1 bg-gray-100 rounded-full text-[11px] text-gray-700">
          ...
          <div className="card p-5 space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1.5">BU *</label>
                <select value={form.bu} onChange={e => set("bu", e.target.value)} className={selectCls}>
                  {["JACQES","CAZA","ADVISOR","VENTURE","ENRD"].map(b => <option key={b}>{b}</option>)}
                </select>
              </div>
              <div>
