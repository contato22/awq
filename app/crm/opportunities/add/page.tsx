...
function AddOpportunityPageInner() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const probability = STAGE_PROBABILITY[form.stage as keyof typeof STAGE_PROBABILITY] ?? 25;
    const accountName = form.account_id ? accounts.find(a => a.account_id === form.account_id)?.account_name : undefined;

    // Always persist to localStorage so the pipeline page sees it immediately
    try {
      const existing = JSON.parse(localStorage.getItem("crm-opportunities-v3") ?? "[]");
      localStorage.setItem("crm-opportunities-v3", JSON.stringify([
        ...existing,
        {
