...
import {
  User, Building2, Phone, Briefcase,
...
function findLead(id: string): CrmLead | null {
  ...
  const found = local.find(l => l.lead_id === id);
  if (found) return found;
  return SEED_LEADS.find(l => l.lead_id === id) ?? null;
}
...
function EditLeadPageInner() {
  ...
  const [form, setForm] = useState<FormData>({
    ...
    bant_budget: "", bant_authority: false, bant_need: "medium",
    bant_timeline: "", qualification_notes: "",
  });

  useEffect(() => {
    if (!leadId) { setNotFound(true); return; }
    const lead = findLead(leadId);
    if (!lead) { setNotFound(true); return; }
    setForm({
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const payload = {
        ...
        bant_need: (form.bant_need || null) as CrmLead["bant_need"],
      ...
      if (!saved) {
        ...
        if (leadId.startsWith("local-")) {
          const local = JSON.parse(localStorage.getItem("awq_local_leads") ?? "[]") as CrmLead[];
          localStorage.setItem("awq_local_leads", JSON.stringify(
            local.map(l => l.lead_id === leadId ? updated : l)
          ));
        }
      }
      ...
      setToast({ message: err instanceof Error ? err.message : "Erro ao salvar lead", type: "error" });
    } finally {
      setSubmitting(false);
    }
  }
  ...
  if (notFound) {
    return (
      <>
        <Header title="Lead não encontrado" subtitle="" />
        <div className="page-container">
          <div className="card p-8 text-center">
            <p className="text-gray-500 text-sm mb-4">Lead não encontrado ou ID inválido.</p>
            <button onClick={() => router.push("/crm/leads")} className="btn-primary text-sm">
              Voltar para Leads
            </button>
          </div>
        </div>
  ...
  return (
    <>
      <Header title="Editar Lead" subtitle="Atualizar dados do prospect" />
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          <FormSection icon={<User size={15} />} title="Dados do Lead">
            ...
            <Field label="E-mail">
              <div className="relative">
                ...
                <input type="email" className={`${inputCls} pl-9`} value={form.email} onChange={e => set("email", e.target.value)} />
              </div>
            </Field>
            <Field label="Telefone">
              <div className="relative">
                <Phone size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
                <input type="tel" className={`${inputCls} pl-9`} value={form.phone} onChange={e => set("phone", e.target.value)} />
              </div>
            </Field>
            <Field label="Cargo" fullWidth>
              <div className="relative">
                ...
                <input type="text" className={`${inputCls} pl-9`} value={form.job_title} onChange={e => set("job_title", e.target.value)} />
              </div>
            </Field>
          </FormSection>
          ...
          <FormSection icon={<Building2 size={15} />} title="Qualificação">
            ...
            <Field label="BU (Unidade de Negócio)" required>
              <select className={`${selectCls} ${errors.bu ? "border-red-400" : ""}`}
          ...
          <div className="flex items-center justify-end gap-3 pt-2">
            ...
            <button type="submit" disabled={submitting}
              ...
              {submitting ? (
                ...
                <><CheckCircle2 size={14} /><BarChart3 size={14} className="hidden" />Salvar Alterações</>
              )}
            </button>
          </div>
...
export default function EditLeadPage() {
  return <Suspense><EditLeadPageInner /></Suspense>;
}
