// ─── /jacqes/assets — JACQES BU Asset Subledger ──────────────────────────────
//
// DATA SOURCE: lib/asset-data.ts (JACQES subledger) via lib/asset-query.ts
// SCOPE: All assets owned/custodied by JACQES Agência Ltda. Q1 2026 snapshot.
// OWNERSHIP: This data is originated and maintained by JACQES.
//            The AWQ Holding receives a read-only consolidated snapshot.
//            This page is the JACQES-level detailed view.
...
function SnapshotBanner() {
  return (
    <div className="flex items-start gap-3 p-3 rounded-lg bg-amber-50 border border-amber-200 text-xs text-amber-700">
      ...
      <span>
        <strong>Snapshot Patrimonial Q1 2026 — JACQES</strong> — dados do subledger de ativos da
        JACQES Agência Ltda (lib/asset-data.ts). Este é o registro originado pelo BU.
    ...
    <div className="card p-5">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-sm font-semibold text-gray-900 flex items-center gap-2">
          ...
          Subledger de Ativos — JACQES
        </h2>
        <span className="text-[10px] text-gray-400">{assets.length} ativo(s)</span>
      </div>
      {assets.length === 0 ? (
        <p className="text-xs text-gray-400 py-6 text-center">
          Nenhum ativo registrado no subledger JACQES para este período.
        </p>
      ) : (
    ...
    return (
      <div className="card p-4 border-l-4 border-emerald-400 flex items-center gap-3">
        ...
        <div className="text-sm text-gray-700">
          <span className="font-semibold">Nenhum alerta de governança</span> para JACQES neste período.
        </div>
      </div>
    );
  }
    ...
    <div className="card p-5 border-l-4 border-amber-400">
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-sm font-semibold text-gray-900 flex items-center gap-2">
          ...
          Alertas de Governança — JACQES ({alerts.length})
        </h2>
        <Link
...
export default async function JacqesAssetsPage() {
  const view = await buildBuAssetView("jacqes");

  return (
    <>
      <Header
        title="Ativos — JACQES"
        subtitle={`Subledger Patrimonial · ${view.snapshot.period} · JACQES Agência Ltda`}
      />
      <div className="page-container">
        ...
        <div className="flex items-start gap-2 p-3 rounded-lg bg-brand-50 border border-brand-200 text-xs text-brand-700">
          ...
          <span>
            <strong>Dados originados pela JACQES.</strong> Este subledger é de propriedade e responsabilidade
            do BU JACQES Agência Ltda. A AWQ Holding recebe uma visão consolidada de leitura — não pode
