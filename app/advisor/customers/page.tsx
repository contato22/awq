...
const typeConfig: Record<string, { color: string; bg: string }> = {
  "Family Office":     { color: "text-violet-400", bg: "bg-violet-500/10"  },
  "PF High Net Worth": { color: "text-brand-400",  bg: "bg-brand-500/10"   },
  "Institucional":     { color: "text-emerald-400", bg: "bg-emerald-500/10" },
  "Empresarial":       { color: "text-amber-400",  bg: "bg-amber-500/10"   },
};

const riscoConfig: Record<string, string> = {
  "Conservador": "text-emerald-400",
  "Moderado":    "text-amber-400",
  "Arrojado":    "text-red-400",
};
    ...
    <>
      ...
      <div className="px-8 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
          {[
            { label: "AUM Total",       value: fmtR(totalAum),  sub: `${ativos} clientes ativos`,  icon: Briefcase, color: "text-violet-400", bg: "bg-violet-500/10", delta: "+18.3%", up: true  },
            { label: "AUM Médio",        value: fmtR(Math.round(totalAum / clients.length)), sub: "por cliente",    icon: DollarSign, color: "text-emerald-400", bg: "bg-emerald-500/10", delta: "+14.1%", up: true  },
            { label: "Retorno Médio",    value: `+${avgReturn}%`, sub: "vs Ibov +9.2%",           icon: TrendingUp, color: "text-brand-400", bg: "bg-brand-500/10", delta: "+5.6pp vs bench", up: true  },
            { label: "NPS Médio",        value: String(avgNps),  sub: `${clients.length} clientes`, icon: Users, color: "text-amber-400", bg: "bg-amber-500/10", delta: "+6pts vs 2025", up: true  },
          ].map((card) => {
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <div className="xl:col-span-2 card p-5">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {clients.map((c) => {
                    const tc = typeConfig[c.type] ?? { color: "text-gray-400", bg: "bg-gray-800" };
                    return (
                      <tr key={c.id} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                        <td className="py-2.5 px-3">
                          <div className="text-xs font-medium text-gray-200">{c.name}</div>
                          <div className="text-[10px] text-gray-600 mt-0.5">desde {c.since}</div>
                        </td>
                        <td className="py-2.5 px-3">
                        ...
                        <td className="py-2.5 px-3 text-right text-xs font-bold text-white">{fmtR(c.aum)}</td>
                        <td className="py-2.5 px-3">
                          <span className={`text-xs font-semibold ${riscoConfig[c.risco]}`}>{c.risco}</span>
                        </td>
                        <td className="py-2.5 px-3 text-right text-xs font-bold text-emerald-400">+{c.retorno}%</td>
                        <td className="py-2.5 px-3 text-right text-xs text-gray-400">{c.fee}% a.a.</td>
                        <td className="py-2.5 px-3 text-right text-xs">
                          <span className={`font-bold ${c.nps >= 80 ? "text-emerald-400" : c.nps >= 70 ? "text-amber-400" : "text-red-400"}`}>
                            {c.nps}
                          </span>
                        </td>
                        <td className="py-2.5 px-3">
                ...
                <tfoot>
                  <tr className="border-t border-gray-700">
                    ...
                    <td />
                    <td className="py-2.5 px-3 text-right text-xs font-bold text-white">{fmtR(totalAum)}</td>
                    <td colSpan={5} />
                  </tr>
                </tfoot>
              </table>
          ...
          <div className="space-y-4">
            <div className="card p-5">
              <h2 className="text-sm font-semibold text-white mb-4">AUM por Tipo de Cliente</h2>
                ...
                return (
                  <div key={type} className="mb-3">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-xs font-bold text-white">{fmtR(aumType)}</span>
                    </div>
                    <div className="h-1.5 bg-gray-800 rounded-full overflow-hidden">
                      <div className={`h-full rounded-full bg-violet-500`} style={{ width: `${barPct}%` }} />
                    </div>
                    <div className="text-[10px] text-gray-600 mt-0.5">{barPct.toFixed(0)}% do total</div>
                  </div>
                );
              })}
            </div>
            ...
            <div className="card p-5">
              <h2 className="text-sm font-semibold text-white mb-3">Perfil de Risco</h2>
              {["Conservador", "Moderado", "Arrojado"].map((risco) => {
                ...
                return (
                  <div key={risco} className="flex items-center justify-between py-2 border-b border-gray-800/50 last:border-0">
                    <div className="flex items-center gap-2">
                      {risco === "Conservador" ? <CheckCircle2 size={12} className="text-emerald-400" /> : <Clock size={12} className="text-amber-400" />}
                      <span className={`text-xs font-medium ${riscoConfig[risco]}`}>{risco}</span>
                    </div>
                    <div className="text-right">
                      <div className="text-xs font-bold text-white">{fmtR(aumR)}</div>
                      <div className="text-[10px] text-gray-600">{count} clientes</div>
                    </div>
                  </div>
                );
              })}
