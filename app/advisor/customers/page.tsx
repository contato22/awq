    ...
    <>
      ...
      <div className="px-8 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
          {[
            { label: "AUM Total",       value: fmtR(totalAum),  sub: `${ativos} clientes ativos`,  icon: Briefcase, color: "text-violet-700", delta: "+18.3%", up: true  },
            { label: "AUM Médio",        value: fmtR(Math.round(totalAum / clients.length)), sub: "por cliente",    icon: DollarSign, color: "text-emerald-600", delta: "+14.1%", up: true  },
            { label: "Retorno Médio",    value: `+${avgReturn}%`, sub: "vs Ibov +9.2%",           icon: TrendingUp, color: "text-brand-600", delta: "+5.6pp vs bench", up: true  },
            { label: "NPS Médio",        value: String(avgNps),  sub: `${clients.length} clientes`, icon: Users, color: "text-amber-700", delta: "+6pts vs 2025", up: true  },
          ].map((card) => {
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <div className="xl:col-span-2 card-elevated p-5">
            <h2 className="text-sm font-bold text-slate-800 mb-4">Carteira de Clientes</h2>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-slate-800">
                    <th className="text-left  py-2 px-3 text-xs font-semibold text-white">Cliente</th>
                    <th className="text-left  py-2 px-3 text-xs font-semibold text-white">Perfil</th>
                    ...
                    <th className="text-right py-2 px-3 text-xs font-semibold text-white">Taxa</th>
                ...
                <tbody>
                  {clients.map((c, idx) => {
                    const tc = typeConfig[c.type] ?? { color: "text-gray-500", bg: "bg-gray-100" };
                    return (
                      <tr key={c.id} className={`border-b border-gray-100 hover:bg-gray-100 transition-colors ${idx % 2 === 1 ? "bg-gray-50/60" : ""}`}>
                        <td className="py-2.5 px-3">
                          <div className="text-xs font-medium text-gray-800">{c.name}</div>
                          <div className="text-[10px] text-gray-500 mt-0.5">desde {c.since}</div>
                        </td>
                        <td className="py-2.5 px-3">
                          <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold border ${tc.bg} ${tc.color}`}>
                            {c.type}
                          </span>
                        </td>
                        <td className="py-2.5 px-3 text-right text-xs font-bold text-emerald-600">{fmtR(c.aum)}</td>
                        <td className="py-2.5 px-3">
                ...
                <tfoot>
                  <tr className="border-t border-gray-300">
                    ...
                    <td />
                    <td className="py-2.5 px-3 text-right text-xs font-bold text-emerald-600">{fmtR(totalAum)}</td>
                    <td colSpan={5} />
                  </tr>
                </tfoot>
              </table>
          ...
          <div className="space-y-4">
            <div className="card-elevated p-5">
              <h2 className="text-sm font-bold text-slate-800 mb-4">AUM por Tipo de Cliente</h2>
                ...
                return (
                  <div key={type} className="mb-3">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-xs font-bold text-emerald-600">{fmtR(aumType)}</span>
                    </div>
                    <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                      <div className="h-full rounded-full bg-gradient-to-r from-slate-600 to-slate-400" style={{ width: `${barPct}%` }} />
                    </div>
                    <div className="text-[10px] text-gray-500 mt-0.5">{barPct.toFixed(0)}% do total</div>
                  </div>
                );
              })}
            </div>
            ...
            <div className="card-elevated p-5">
              <h2 className="text-sm font-bold text-slate-800 mb-3">Perfil de Risco</h2>
                ...
                return (
                  <div key={risco} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                    ...
                    <div className="text-right">
                      <div className="text-xs font-bold text-emerald-600">{fmtR(aumR)}</div>
                      <div className="text-[10px] text-gray-500">{count} clientes</div>
                    </div>
                  </div>
                );
              })}
