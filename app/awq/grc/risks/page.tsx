...
const EMPTY = { name: "", category: "", probability: "Médio" as string, impact: "Médio" as string, status: "Aberto" as string, owner: "", mitigation: "" };

export default function RiscosPage() {
  const [data, setData] = useState<GrcRisk[]>([]);
  ...
  useEffect(() => {
    fetch("/api/grc/risks").then(r => r.json()).then(d => { setData(d.data ?? []); setLoading(false); });
  }, []);

  async function onCreate(e: React.FormEvent) {
    e.preventDefault();
    setSaving(true);
    const r = await fetch("/api/grc/risks", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
  ...
  async function onDelete(id: string) {
    await fetch("/api/grc/risks", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id }) });
    setData(p => p.filter(x => x.id !== id));
  }

  const badge = (v: string) => {
    const map: Record<string, string> = { Alto: "bg-red-100 text-red-700", Médio: "bg-yellow-100 text-yellow-700", Baixo: "bg-green-100 text-green-700", Aberto: "bg-blue-100 text-blue-700", Mitigado: "bg-gray-100 text-gray-700" };
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  {["Nome", "Categoria", "Probabilidade", "Impacto", "Status", "Responsável", ""].map(h => (
              ...
              <tbody className="divide-y divide-gray-100">
                {data.map(r => (
                  <tr key={r.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3"><span className={badge(r.probability)}>{r.probability}</span></td>
                    <td className="px-4 py-3"><span className={badge(r.impact)}>{r.impact}</span></td>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
            ...
            <form onSubmit={onCreate} className="px-6 py-4 space-y-3">
              ...
              {[["Probabilidade", "probability"], ["Impacto", "impact"], ["Status", "status"]].map(([label, key]) => (
                <div key={key}>
                  <label className="block text-xs font-medium text-gray-700 mb-1">{label}</label>
                  <select className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" value={form[key as keyof typeof form]} onChange={e => setForm(p => ({ ...p, [key]: e.target.value }))}>
                    {key === "status" ? ["Aberto", "Mitigado", "Aceito", "Fechado"].map(o => <option key={o}>{o}</option>) : ["Alto", "Médio", "Baixo"].map(o => <option key={o}>{o}</option>)}
                  </select>
                </div>
              ))}
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Mitigação</label>
                <textarea className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" rows={3} value={form.mitigation} onChange={e => setForm(p => ({ ...p, mitigation: e.target.value }))} />
              </div>
