...
const EMPTY = { name: "", description: "", status: "Ativo" as string, owner: "", risk_id: null as string | null };

export default function ControlesPage() {
  const [data, setData] = useState<GrcControl[]>([]);
  ...
  useEffect(() => {
    fetch("/api/grc/controls").then(r => r.json()).then(d => { setData(d.data ?? []); setLoading(false); });
  }, []);

  async function onCreate(e: React.FormEvent) {
    e.preventDefault();
    setSaving(true);
    const r = await fetch("/api/grc/controls", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
  ...
  async function onDelete(id: string) {
    await fetch("/api/grc/controls", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id }) });
    setData(p => p.filter(x => x.id !== id));
  }

  const badge = (v: string) => {
    const map: Record<string, string> = { Ativo: "bg-green-100 text-green-700", Inativo: "bg-gray-100 text-gray-600" };
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  {["Nome", "Descrição", "Status", "Responsável", ""].map(h => (
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
            <div className="px-6 py-4 border-b border-gray-200">
              <h2 className="text-base font-semibold text-gray-900">Novo Controle</h2>
            </div>
            <form onSubmit={onCreate} className="px-6 py-4 space-y-3">
              ...
              <div>
                ...
                <select className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" value={form.status} onChange={e => setForm(p => ({ ...p, status: e.target.value }))}>
                  {["Ativo", "Inativo"].map(o => <option key={o}>{o}</option>)}
                </select>
              </div>
              <div>
