...
// usuário interno). Gate no middleware; standalone (LayoutShell bare). UI clara,
// alinhada à AWQ Platform (brand-*). Seções: BI · GRADE · COMISSÕES. Sem dado
// financeiro interno (GMV/MC/ROIC/Net) — comissões = host/creator (operacional).
...
function Stat({ icon, value, label }: { icon: React.ReactNode; value: string; label: string }) {
  return (
    <div className="card p-5 text-center">
      <div className="mb-1 flex justify-center text-brand-600">{icon}</div>
...
function SectionTitle({ icon, children }: { icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <h2 className="mb-4 flex items-center justify-center gap-2 text-xs font-semibold uppercase tracking-widest text-gray-400">
      {icon} {children}
    </h2>
  );
}
...
function Detail({ icon, label, children }: { icon: React.ReactNode; label: string; children: React.ReactNode }) {
  return (
    <div className="text-xs text-gray-600">
      <p className="mb-1 flex items-center gap-1.5 text-[10px] uppercase tracking-wide text-gray-400">{icon} {label}</p>
      {children}
    </div>
  );
}
...
export default async function LiveShopPortalPage({ params }: { params: { brand: string } }) {
  ...
  const biNode = (
    <div className="space-y-10">
      ...
      <section>
        ...
        <div className="card p-4"><LiveShopPublicBI events={data.events} /></div>
      ...
      <section>
        ...
        {data.events.length === 0 ? (
          <p className="text-center text-sm text-gray-400">Nenhuma live realizada ainda.</p>
        ) : (
          <div className="space-y-2">
            {data.events.map((e) => (
              <div key={e.id} className="card flex items-center gap-4 px-5 py-3.5">
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-brand-50 text-brand-600"><Radio size={15} /></div>
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-medium text-gray-900">{fmtDateTime(e.startedAt)}</p>
                  <p className="flex items-center gap-1.5 text-xs text-gray-400"><Clock size={11} /> {fmtDuration(e.durationMin)}</p>
                </div>
                <div className="hidden gap-6 text-right sm:flex">
                  <div><p className="text-sm font-semibold text-gray-900">{fmtInt(e.views)}</p><p className="text-[10px] uppercase tracking-wide text-gray-400">views</p></div>
                  <div><p className="text-sm font-semibold text-gray-900">{fmtInt(e.peakCcv)}</p><p className="text-[10px] uppercase tracking-wide text-gray-400">pico</p></div>
                </div>
              </div>
            ))}
          </div>
  ...
  // ── GRADE (calendário + agenda) ───────────────────────────────────────────────
  const gradeNode = (
    <div className="space-y-10">
      <section>
        <SectionTitle icon={<Calendar size={13} />}>Calendário</SectionTitle>
        <LiveShopCalendar entries={data.calendar} dark={false} />
      </section>
            ...
            {data.agenda.map((p) => (
              <div key={p.id} className="card p-5">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div>
                    <p className="text-[11px] uppercase tracking-wide text-gray-400">{fmtDateTime(p.startsAt)}</p>
                    <p className="text-base font-semibold text-gray-900">{p.title}</p>
                  </div>
                  <span className="rounded-full bg-brand-50 px-2.5 py-0.5 text-[11px] font-medium text-brand-700">{LIVE_PLAN_STATUS_LABEL[p.status]}</span>
                </div>
                <div className="mt-4 grid gap-4 sm:grid-cols-2">
                  <Detail icon={<Package size={12} />} label="Peças"><ul className="space-y-0.5">{p.pecas.map((x, i) => <li key={i}>· {x}</li>)}</ul></Detail>
                  <Detail icon={<UserCircle size={12} />} label="Responsáveis"><ul className="space-y-0.5">{p.responsaveis.map((r, i) => <li key={i}>{r.role}: <span className="text-gray-800">{r.name}</span></li>)}</ul></Detail>
  ...
  const comissoesNode = (
    <section>
      ...
      {data.agenda.every((p) => p.comissoes.length === 0) ? (
        <p className="text-center text-sm text-gray-400">Sem comissões cadastradas.</p>
      ) : (
          ...
          {data.agenda.map((p) => (
            <div key={p.id} className="card p-5">
              <div className="mb-3 flex items-center justify-between gap-2">
                ...
                <p className="text-[11px] text-gray-400">{fmtDateTime(p.startsAt)}</p>
              </div>
  ...
  return (
    <div className="min-h-screen bg-surface-subtle text-gray-900">
      <main className="mx-auto max-w-3xl px-6 py-14">
        <div className="mb-10 text-center">
          ...
          <h1 className="text-3xl font-bold tracking-tight text-brand-800 sm:text-4xl">{data.brand.name}</h1>
          <p className="mx-auto mt-3 max-w-xl text-sm text-gray-500">{data.brand.segment} · {data.brand.kind}</p>
        </div>
        ...
        <footer className="mt-16 text-center text-xs text-gray-400">© {new Date().getFullYear()} AWQ Group · Live Shop</footer>
      </main>
    </div>
  );
}
