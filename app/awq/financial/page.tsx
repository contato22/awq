      ...
      <div className="px-8 py-6 space-y-6">
              ...
              <div key={card.label} className="card p-5 flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1 mt-1">
                    ...
                    <span className="text-[10px] font-semibold text-emerald-400">{card.delta}</span>
        ...
        <div className="card p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-semibold text-white">DRE por BU — Jan–Mar 2026 (YTD)</h2>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-800">
                  ...
                  <th className="text-right py-2 px-3 text-xs font-semibold text-brand-400">JACQES</th>
                  <th className="text-right py-2 px-3 text-xs font-semibold text-emerald-400">Caza Vision</th>
                  <th className="text-right py-2 px-3 text-xs font-semibold text-violet-400">Advisor</th>
                  <th className="text-right py-2 px-3 text-xs font-semibold text-gray-300">TOTAL</th>
              ...
              <tbody>
                {plLines.map((row, i) => {
                  ...
                  return (
                    <tr
                      key={i}
                      className={`border-b border-gray-800/50 ${isSubtotal ? "bg-gray-800/20" : "hover:bg-gray-800/30"} transition-colors`}
                    >
                      <td
                        className={`py-2 px-3 text-xs ${isSubtotal ? "font-bold text-gray-100" : "text-gray-400"}`}
                      ...
                      {[row.jacqes, row.caza, row.advisor, row.total].map((v, j) => (
                        <td
                          ...
                          className={`py-2 px-3 text-right text-xs ${isSubtotal ? "font-bold" : ""} ${
                            isNeg ? "text-red-400" : isSubtotal ? "text-white" : "text-gray-300"
                          }`}
                        >
                          {fmtR(v)}
                        </td>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          ...
          {/* ── Revenue mix ──────────────────────────────────────────────────── */}
          <div className="card p-5">
            <h2 className="text-sm font-semibold text-white mb-4">Mix de Receita por BU</h2>
            ...
            <div className="mt-5 pt-4 border-t border-gray-800">
              ...
              <div className="grid grid-cols-3 gap-2">
                {operatingBus.map((bu) => (
                  <div key={bu.id} className="text-center p-3 rounded-lg bg-gray-800/30">
                    <div className={`text-base font-bold ${bu.accentColor}`}>{bu.name}</div>
                    <div className="text-xs text-gray-500 mt-1">Bruta: <span className="text-white font-semibold">{pct(bu.grossProfit, bu.revenue)}</span></div>
                    <div className="text-xs text-gray-500">EBITDA: <span className="text-brand-400 font-semibold">{pct(bu.ebitda, bu.revenue)}</span></div>
                    <div className="text-xs text-gray-500">Líq.: <span className="text-emerald-400 font-semibold">{pct(bu.netIncome, bu.revenue)}</span></div>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* ── Monthly Revenue ───────────────────────────────────────────────── */}
          <div className="card p-5">
            <h2 className="text-sm font-semibold text-white mb-4">Receita Mensal — Consolidado</h2>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-800">
                    <th className="text-left  py-2 px-2 text-xs font-semibold text-gray-500">Mês</th>
                    <th className="text-right py-2 px-2 text-xs font-semibold text-brand-400">JACQES</th>
                    <th className="text-right py-2 px-2 text-xs font-semibold text-emerald-400">Caza</th>
                    <th className="text-right py-2 px-2 text-xs font-semibold text-violet-400">Advisor</th>
                    <th className="text-right py-2 px-2 text-xs font-semibold text-white">Total</th>
                  </tr>
                </thead>
                <tbody>
                  {monthlyRevenue.map((row) => {
                    ...
                    return (
                      <tr key={row.month} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                        <td className="py-2.5 px-2 text-xs font-medium text-gray-300">{row.month}</td>
                        ...
                        <td className="py-2.5 px-2 text-right text-xs font-bold text-white">
                          {fmtR(row.total)}
                          {growth && (
                            <span className="text-[10px] text-emerald-400 ml-1">+{growth}%</span>
                          )}
                        </td>
                      </tr>
                    );
                ...
                <tfoot>
                  <tr className="border-t border-gray-700">
                    ...
                    <td className="py-2.5 px-2 text-right text-xs font-bold text-brand-400">
                      {fmtR(monthlyRevenue.reduce((s, r) => s + r.jacqes, 0))}
                    </td>
                    <td className="py-2.5 px-2 text-right text-xs font-bold text-emerald-400">
                      {fmtR(monthlyRevenue.reduce((s, r) => s + r.caza, 0))}
                    </td>
                    <td className="py-2.5 px-2 text-right text-xs font-bold text-violet-400">
            ...
            <div className="mt-4 pt-3 border-t border-gray-800 space-y-1">
              <div className="text-[10px] font-bold text-gray-600 uppercase tracking-widest mb-2">P&L por BU</div>
              {buData.map((bu) => (
                <Link
                  key={bu.id}
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[11px] text-gray-400 group-hover:text-white transition-colors">{bu.name}</span>
                  </div>
