...
import type { FiscalPeriod } from "@/lib/epm-db";

function buildChecklist(done: boolean[], notes?: (string | undefined)[]) {
  return [
  ...
  ].map((label, i) => ({ label, done: done[i] ?? false, note: notes?.[i] }));
}

const SEED: FiscalPeriod[] = [
  { id: "1",  period_code: "2026-01", period_type: "MONTH",   start_date: "2026-01-01", end_date: "2026-01-31", status: "LOCKED",    closed_by: "CFO", closed_at: "2026-02-05T09:00:00Z", checklist: buildChecklist([true, true, true, true, true, true]) },
  { id: "2",  period_code: "2026-02", period_type: "MONTH",   start_date: "2026-02-01", end_date: "2026-02-28", status: "LOCKED",    closed_by: "CFO", closed_at: "2026-03-04T10:30:00Z", checklist: buildChecklist([true, true, true, true, true, true]) },
  { id: "3",  period_code: "2026-03", period_type: "MONTH",   start_date: "2026-03-01", end_date: "2026-03-31", status: "CLOSED",    closed_by: "CFO", closed_at: "2026-04-03T14:00:00Z", checklist: buildChecklist([true, true, true, true, true, false], [, , , , , "Provisão férias pendente"]) },
  { id: "4",  period_code: "2026-Q1", period_type: "QUARTER", start_date: "2026-01-01", end_date: "2026-03-31", status: "CLOSED",    closed_by: "CEO", closed_at: "2026-04-08T11:00:00Z", checklist: buildChecklist([true, true, true, true, true, true]) },
  { id: "5",  period_code: "2026-04", period_type: "MONTH",   start_date: "2026-04-01", end_date: "2026-04-30", status: "REVIEWING", checklist: buildChecklist([true, true, false, true, false, false], [, , "3 faturas AP pendentes", , "Deprec. abril não lançada"]) },
  { id: "6",  period_code: "2026-05", period_type: "MONTH",   start_date: "2026-05-01", end_date: "2026-05-31", status: "OPEN",      checklist: buildChecklist([false, false, false, false, false, false]) },
  { id: "7",  period_code: "2026-06", period_type: "MONTH",   start_date: "2026-06-01", end_date: "2026-06-30", status: "OPEN",      checklist: buildChecklist([false, false, false, false, false, false]) },
  { id: "8",  period_code: "2026-Q2", period_type: "QUARTER", start_date: "2026-04-01", end_date: "2026-06-30", status: "OPEN",      checklist: buildChecklist([false, false, false, false, false, false]) },
  { id: "9",  period_code: "2026-07", period_type: "MONTH",   start_date: "2026-07-01", end_date: "2026-07-31", status: "OPEN",      checklist: buildChecklist([false, false, false, false, false, false]) },
  { id: "10", period_code: "2026-08", period_type: "MONTH",   start_date: "2026-08-01", end_date: "2026-08-31", status: "OPEN",      checklist: buildChecklist([false, false, false, false, false, false]) },
  { id: "11", period_code: "2026-09", period_type: "MONTH",   start_date: "2026-09-01", end_date: "2026-09-30", status: "OPEN",      checklist: buildChecklist([false, false, false, false, false, false]) },
  { id: "12", period_code: "2026-Q3", period_type: "QUARTER", start_date: "2026-07-01", end_date: "2026-09-30", status: "OPEN",      checklist: buildChecklist([false, false, false, false, false, false]) },
];
...
export default function PeriodsPage() {
  const [periods, setPeriods] = useState<FiscalPeriod[]>(SEED);
  const [isSeed, setIsSeed]   = useState(true);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch("/api/awq/epm/periods")
      .then(r => r.json())
      .then(j => { if (j.success && j.data.length > 0) { setPeriods(j.data); setIsSeed(false); } })
      .catch(() => {})
  ...
  const counts = {
    ...
    REVIEWING: periods.filter(p => p.status === "REVIEWING").length,
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="max-w-screen-xl mx-auto flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-gray-900">Fechamento de Períodos</h1>
            <p className="text-xs text-gray-500">EPM · Fiscal Periods · Close & Lock · FY2026</p>
          </div>
          {isSeed && (
      ...
      <div className="max-w-screen-xl mx-auto px-6 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {(["LOCKED", "CLOSED", "REVIEWING", "OPEN"] as PeriodStatus[]).map(status => {
            ...
            return (
              <div key={status} className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-3">
                ...
                <div>
                  <div className={`text-xl font-bold ${color}`}>{counts[status]}</div>
        ...
        {reviewing && (
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-sm font-semibold text-gray-900">Período em Revisão: {reviewing.period_code}</span>
              ...
              {reviewing.checklist.map(item => (
                <div key={item.label} className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <div className={`text-xs font-medium ${item.done ? "text-gray-700" : "text-amber-700"}`}>{item.label}</div>
                    {item.note && <div className="text-[11px] text-amber-600 mt-0.5">{item.note}</div>}
                  </div>
            ...
            {reviewing.checklist.some(c => !c.done) && (
              <div className="mt-2 text-[11px] text-amber-600">Resolva todos os itens pendentes antes de fechar o período.</div>
            )}
          </div>
        )}
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {months.map(p => {
                const cfg    = STATUS_CFG[p.status as PeriodStatus];
                const { Icon } = cfg;
                ...
                return (
                  <tr key={p.period_code} className="border-b border-gray-50 hover:bg-gray-50">
                    ...
                    <td className="py-2.5 px-3">
                      <div className="flex items-center gap-1.5">
                        <div className="h-1.5 w-16 bg-gray-100 rounded-full overflow-hidden">
                          <div className={`h-full rounded-full ${allDone ? "bg-emerald-400" : "bg-amber-400"}`} style={{ width: `${(doneN / totalN) * 100}%` }} />
                        </div>
                    ...
                    <td className="py-2.5 px-3">
                      ...
                      {p.status === "LOCKED" && (
                        <span className="flex items-center gap-1 text-[10px] text-violet-500"><Lock size={9} /> Bloqueado</span>
                      )}
                    </td>
                  </tr>
                );
            ...
            {quarters.map(p => {
              ...
              return (
                <div key={p.period_code} className={`rounded-xl p-4 border ${p.status === "LOCKED" ? "border-violet-200 bg-violet-50" : p.status === "CLOSED" ? "border-brand-200 bg-brand-50" : "border-gray-200 bg-gray-50"}`}>
                  ...
                  <div className="text-[10px] text-gray-400">Checklist: {doneN}/{totalN} ✓</div>
                  {p.closed_by && <div className="text-[10px] text-gray-400 mt-1">Fechado por {p.closed_by}</div>}
                </div>
              );
            })}
          </div>
        ...
        <div className="flex items-start gap-3 p-4 bg-brand-50 border border-brand-200 rounded-xl text-xs text-brand-800">
          ...
          <div>
            ...
            Períodos BLOQUEADOS não aceitam novos lançamentos GL.
          </div>
        </div>
