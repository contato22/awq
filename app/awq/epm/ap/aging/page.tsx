...
import type { APItem, AgingBucket } from "@/lib/ap-ar-db";
...
export default function APAgingPage() {
  ...
  useEffect(() => {
    fetch("/api/epm/ap?status=OVERDUE")
      .then((r) => r.json())
  ...
  const bucketMap = BUCKETS.reduce<Record<AgingBucket, APItem[]>>((acc, b) => {
    acc[b] = overdue.filter((i) => getAgingBucket(i.due_date) === b);
    return acc;
  }, {} as Record<AgingBucket, APItem[]>);
  ...
  const bySupplier = displayed.reduce<Record<string, { items: APItem[]; total: number }>>((acc, i) => {
    if (!acc[i.supplier_name]) acc[i.supplier_name] = { items: [], total: 0 };
    acc[i.supplier_name].items.push(i);
    acc[i.supplier_name].total += i.net_amount;
    return acc;
  }, {});
  ...
  return (
    <div className="p-6 space-y-6 max-w-6xl mx-auto">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">AP Aging — Contas a Pagar Vencidas</h1>
        <p className="text-sm text-gray-500 mt-1">Visão por faixa de vencimento e fornecedor</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {Object.entries(bySupplier)
            ...
            .map(([supplier, data]) => (
              <div key={supplier} className="bg-white rounded-xl border shadow-sm overflow-hidden">
