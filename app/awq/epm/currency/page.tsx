...
import {
  ...
  RefreshCw, BarChart3, ArrowUpDown, ArrowLeft,
} from "lucide-react";
import type { FXRate, FCTransaction } from "@/lib/epm-db";
...
const SEED_FX: FXRate[] = [
  { id: "1", currency: "USD", symbol: "$",  flag: "🇺🇸", rate_brl: 5.1240, rate_prev: 5.0820, source: "BCB PTAX", as_of: "2026-04-30" },
  { id: "2", currency: "EUR", symbol: "€",  flag: "🇪🇺", rate_brl: 5.8760, rate_prev: 5.8120, source: "BCB PTAX", as_of: "2026-04-30" },
  { id: "3", currency: "GBP", symbol: "£",  flag: "🇬🇧", rate_brl: 6.4320, rate_prev: 6.3980, source: "BCB PTAX", as_of: "2026-04-30" },
  { id: "4", currency: "ARS", symbol: "$A", flag: "🇦🇷", rate_brl: 0.0052, rate_prev: 0.0058, source: "BCB PTAX", as_of: "2026-04-30" },
  { id: "5", currency: "CHF", symbol: "Fr", flag: "🇨🇭", rate_brl: 5.7840, rate_prev: 5.7210, source: "BCB PTAX", as_of: "2026-04-30" },
];

const SEED_FC: FCTransaction[] = [
  ...
  { id: "FC-006", date: "2026-02-15", entity: "JACQES", description: "Consultoria internacional — EU",    currency: "EUR", amount_fc: 8_500, rate_at_booking: 5.780, rate_current: 5.876, type: "REVENUE",  category: "Receita"      },
...
function calcGainLoss(t: FCTransaction): number {
  ...
  if (t.type === "EXPENSE" || t.type === "ASSET") return bookingBRL - currentBRL;
  return currentBRL - bookingBRL;
}

function calcExposure(txs: FCTransaction[]) {
  ...
  for (const t of txs) {
    ...
    const ex  = map.get(t.currency) ?? { revenue: 0, expense: 0, asset: 0, gainLoss: 0 };
    if (t.type === "REVENUE") ex.revenue += brl;
    if (t.type === "EXPENSE") ex.expense += brl;
    if (t.type === "ASSET")   ex.asset   += brl;
    ex.gainLoss += gl;
    map.set(t.currency, ex);
  }
  return [...map.entries()].map(([currency, v]) => ({ currency, ...v })).sort((a, b) => b.revenue + b.asset - a.revenue - a.asset);
}

export default function CurrencyPage() {
  const [fxRates, setFxRates]       = useState<FXRate[]>(SEED_FX);
  const [fcTxs, setFcTxs]           = useState<FCTransaction[]>(SEED_FC);
  const [isSeed, setIsSeed]         = useState(true);
  const [loading, setLoading]       = useState(true);

  useEffect(() => {
    fetch("/api/awq/epm/currency")
      .then(r => r.json())
      .then(j => {
        if (j.success && (j.fxRates.length > 0 || j.fcTransactions.length > 0)) {
          if (j.fxRates.length > 0) setFxRates(j.fxRates);
          if (j.fcTransactions.length > 0) setFcTxs(j.fcTransactions);
          setIsSeed(false);
        }
      })
      .catch(() => {})
  ...
  const exposureByCurrency = calcExposure(fcTxs);
  const totalGainLoss      = fcTxs.reduce((s, t) => s + calcGainLoss(t), 0);
  const totalFCRevenueBRL  = fcTxs.filter(t => t.type === "REVENUE").reduce((s, t) => s + t.amount_fc * t.rate_current, 0);
  const totalFCExpenseBRL  = fcTxs.filter(t => t.type === "EXPENSE").reduce((s, t) => s + t.amount_fc * t.rate_current, 0);
  const totalFCAssetBRL    = fcTxs.filter(t => t.type === "ASSET").reduce((s, t) => s + t.amount_fc * t.rate_current, 0);
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="max-w-screen-xl mx-auto flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-gray-900">Câmbio & Currency Translation</h1>
            <p className="text-xs text-gray-500">EPM · Multi-currency · FX Rates · Translation Gain/Loss</p>
          </div>
          {isSeed && (
      ...
      <div className="max-w-screen-xl mx-auto px-6 py-6 space-y-6">
        ...
        <div className="flex items-start gap-3 p-4 bg-brand-50 border border-brand-200 rounded-xl text-xs text-brand-800">
          ...
          <div>
            ...
            O grupo opera primariamente em BRL, mas tem exposição em USD (software, receitas internacionais,
            investimentos) e EUR (consultoria EU). Taxas PTAX BCB.
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            { label: "Ganho/Perda Câmbio (YTD)",      value: fmtBRL(totalGainLoss),    color: totalGainLoss >= 0 ? "text-emerald-700" : "text-red-700" },
            { label: "Moedas com Exposição",           value: new Set(fcTxs.map(t => t.currency)).size, color: "text-brand-700" },
          ].map(card => (
            ...
            {fxRates.map(fx => {
              ...
              return (
                <div key={fx.currency} className="bg-gray-50 rounded-xl p-3">
                  ...
                  <div className="text-lg font-bold text-gray-900 tabular-nums">R${fmtFX(fx.rate_brl, 4)}</div>
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {exposureByCurrency.map(row => {
                const rate  = fxRates.find(r => r.currency === row.currency);
                ...
                return (
                  <tr key={row.currency} className="border-b border-gray-50 hover:bg-gray-50">
                    ...
                    <td className="py-2.5 px-3 text-right tabular-nums text-emerald-700 font-semibold">{row.revenue > 0 ? fmtBRL(row.revenue) : "—"}</td>
                    <td className="py-2.5 px-3 text-right tabular-nums text-red-700 font-semibold">{row.expense > 0 ? fmtBRL(row.expense) : "—"}</td>
                    <td className="py-2.5 px-3 text-right tabular-nums text-brand-700">{row.asset > 0 ? fmtBRL(row.asset) : "—"}</td>
            ...
            <tfoot>
              <tr className="border-t-2 border-gray-200 bg-gray-50 font-bold text-xs">
                ...
                <td className="py-2.5 px-3 text-right tabular-nums text-brand-700">{fmtBRL(totalFCAssetBRL)}</td>
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="flex items-center gap-2 px-5 py-4 border-b border-gray-100">
            ...
            <span className="text-sm font-semibold text-gray-900">Transações em Moeda Estrangeira</span>
            <span className="ml-auto text-xs text-gray-400">{fcTxs.length} transações · YTD 2026</span>
          </div>
                  ...
                  return (
                    <tr key={t.id} className="border-b border-gray-50 hover:bg-gray-50">
                      ...
                      <td className="py-2 px-3">
                        <span className="text-[10px] px-2 py-0.5 bg-brand-100 text-brand-700 rounded-full font-bold">{t.currency}</span>
                      </td>
                      <td className="py-2 px-3 text-right tabular-nums text-gray-700">
                        {t.amount_fc.toLocaleString("pt-BR", { minimumFractionDigits: 0 })}
                      </td>
                      <td className="py-2 px-3 text-right tabular-nums text-gray-400">{fmtFX(t.rate_at_booking, 4)}</td>
        ...
        <div className="flex items-center gap-2 px-4 py-3 bg-amber-50 border border-amber-200 rounded-xl text-xs text-amber-700">
          ...
          Taxas de câmbio devem ser atualizadas mensalmente via BCB PTAX. Para exposições significativas, considere hedge cambial.
        </div>
