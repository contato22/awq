...
import { ArrowLeft, Plus, Search, ShoppingBag, Trash2 } from "lucide-react";
...
const EMPTY = { type: "Venda", customer: "", order_date: "", status: "Novo" as string, total: 0, owner: "", notes: "" };
...
export default function SalesOrdersPage() {
  ...
  useEffect(() => {
    fetch("/api/erp/orders").then(r => r.json()).then(d => { setData(d.data ?? []); setLoading(false); });
  }, []);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-screen-xl mx-auto px-6 py-6 space-y-5">
        <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-lg px-3 py-2 max-w-sm shadow-sm">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar cliente…" className="flex-1 text-sm focus:outline-none bg-transparent" />
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-100">
              <thead className="bg-gray-50">
                <tr>
                  {["Cliente", "Tipo", "Data", "Total", "Responsável", "Status", ""].map(h => (
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
            <div className="px-6 py-4 border-b border-gray-200">
              <h2 className="text-base font-semibold text-gray-900">Novo Pedido de Venda</h2>
            </div>
            <form onSubmit={onCreate} className="px-6 py-4 space-y-3">
              {[["Cliente", "customer", "text"], ["Responsável", "owner", "text"], ["Data", "order_date", "date"]].map(([label, key, type]) => (
                <div key={key}>
                  <label className="block text-xs font-medium text-gray-700 mb-1">{label}</label>
                  <input type={type} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" value={form[key as keyof typeof form] as string} onChange={e => setForm(p => ({ ...p, [key]: e.target.value }))} required={key === "customer"} />
                </div>
              ))}
              <div>
