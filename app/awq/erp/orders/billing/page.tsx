...
export default function BillingPage() {
  ...
  useEffect(() => {
    fetch("/api/awq/erp/sales")
      .then(r => r.json())
      .then(j => { if (j.success) setOrders(j.data.filter((o: SalesOrder) => o.status === "Faturado")); })
      .catch(() => {})
  ...
  const valorMedio = orders.length ? valorTotal / orders.length : 0;
        ...
        <div className="max-w-screen-xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">ERP · Pedidos · Faturamento</p>
            </div>
          </div>
      ...
      <div className="max-w-screen-xl mx-auto px-6 py-6 space-y-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
            <div className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Total Faturado</div>
            <div className="text-2xl font-bold text-gray-900">{loading ? "—" : orders.length}</div>
          </div>
          <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
            <div className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Valor Total</div>
            <div className="text-2xl font-bold text-emerald-600">{loading ? "—" : fmtBRL(valorTotal)}</div>
          </div>
          <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
            <div className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Valor Médio</div>
            <div className="text-2xl font-bold text-brand-600">{loading ? "—" : fmtBRL(valorMedio)}</div>
          </div>
        </div>
