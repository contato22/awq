...
export default function ShippingPage() {
  ...
  useEffect(() => {
    fetch("/api/awq/erp/sales")
      .then(r => r.json())
      .then(j => { if (j.success) setOrders(j.data.filter((o: SalesOrder) => o.status === "Entregue")); })
      .catch(() => {})
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="max-w-screen-xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-gray-900">Expedição</h1>
              <p className="text-xs text-gray-500">ERP · Pedidos · Expedição</p>
            </div>
          </div>
      ...
      <div className="max-w-screen-xl mx-auto px-6 py-6 space-y-5">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
            <div className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Pedidos Expedidos</div>
            <div className="text-2xl font-bold text-purple-600">{loading ? "—" : orders.length}</div>
          </div>
          <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
