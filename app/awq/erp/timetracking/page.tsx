...
// ─── /awq/erp/timetracking — ERP Time Tracking (mirrors PPM Timesheets) ──────

import { useState, useEffect, useCallback } from "react";
import Link from "next/link";
import { ArrowLeft, Plus, Clock, CheckCircle2, Save, X, Download } from "lucide-react";
...
export default function ErpTimeTrackingPage() {
  ...
  function exportCSV() {
    ...
    a.href     = url;
    a.download = `erp-timetracking-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
      ...
      <div className="max-w-screen-xl mx-auto px-6 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            { label: "Aguardando Aprovação", value: String(pending),                color: pending > 0 ? "text-amber-600" : "text-gray-900" },
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
            ...
            <form onSubmit={(e) => void submitEntry(e)} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <select
                  value={form.is_billable ? "true" : "false"}
                  onChange={(e) => setForm((f) => ({ ...f, is_billable: e.target.value === "true" }))}
                  className={INPUT}>
                  ...
                  <tr>
                    <td colSpan={10} className="px-4 py-16">
                      <div className="flex flex-col items-center gap-3 text-center">
                        ...
                        <p className="text-sm font-medium text-gray-500">Nenhum apontamento encontrado</p>
