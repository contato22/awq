...
function TypeBadge({ type }: { type: SourceType }) {
  ...
  return (
    <span className={"inline-block px-2 py-0.5 rounded text-xs font-semibold " + map[type]}>
      {type}
    </span>
  );
}
...
function ConfidenceBadge({ level }: { level: ConfidenceLevel }) {
  ...
  return (
    <span className={"inline-block px-2 py-0.5 rounded text-xs font-semibold " + map[level]}>
      {labels[level]}
    </span>
  );
}
...
function ValidationBadge({ v }: { v: LayerValidationStatus }) {
  ...
  const b = map[v];
  return <span className={"inline-block px-2 py-0.5 rounded text-xs font-semibold " + b.cls}>{b.label}</span>;
}
...
function PriorityBadge({ p }: { p: ActionItem["priority"] }) {
  ...
  return <span className={"inline-block px-2 py-0.5 rounded text-xs font-semibold " + map[p]}>{p.toUpperCase()}</span>;
}
      ...
      <div className="page-container">
        ...
        <div className="card p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-base font-semibold text-gray-900">Fontes de Dados da Plataforma</h2>
            <span className="ml-auto text-xs text-gray-400">{DATA_SOURCES.length} fontes catalogadas</span>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-gray-200">
                  <th className="text-left py-2 px-2 font-semibold text-gray-500 uppercase tracking-wide">Fonte</th>
                  <th className="text-left py-2 px-2 font-semibold text-gray-500 uppercase tracking-wide">Tipo</th>
                  <th className="text-left py-2 px-2 font-semibold text-gray-500 uppercase tracking-wide">Escopo</th>
                  <th className="text-left py-2 px-2 font-semibold text-gray-500 hidden md:table-cell uppercase tracking-wide">Descricao</th>
                  <th className="text-center py-2 px-2 font-semibold text-gray-500 uppercase tracking-wide">Consumida</th>
                  <th className="text-center py-2 px-2 font-semibold text-gray-500 uppercase tracking-wide">Confianca</th>
                </tr>
              </thead>
              <tbody>
                {DATA_SOURCES.map((src) => (
                  <tr key={src.name} className="border-b border-gray-100 hover:bg-gray-50/60">
                    <td className="py-2 px-2">
                      <code className="text-xs text-gray-700 font-mono">{src.name}</code>
        ...
        <div className="card p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-base font-semibold text-gray-900">Storage / Persistencia</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-4">
              ...
              <div className="mt-3 rounded bg-red-100 px-2 py-1 text-xs text-red-700 font-medium">
        ...
        <div className="card p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-base font-semibold text-gray-900">Mapa de Consumo — Paginas vs. Fonte</h2>
            <span className="ml-auto text-xs text-gray-400">{activeRoutes.length} rotas no sidebar</span>
          </div>
            ...
            <div className="rounded-lg border border-amber-200 bg-amber-50 p-3">
              ...
              <div className="space-y-0.5">
                {snapshotRoutes.slice(0, 8).map((r) => (
                  <div key={r.href} className="text-xs text-amber-700 font-mono">{r.href}</div>
                ))}
                {snapshotRoutes.length > 8 && (
                  <div className="text-xs text-amber-500">+{snapshotRoutes.length - 8} mais</div>
                )}
              </div>
            </div>
            <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-3">
              ...
              <div className="space-y-0.5">
                {pipelineRoutes.map((r) => (
                  <div key={r.href} className="text-xs text-emerald-700 font-mono">{r.href}</div>
                ))}
                {pipelineRoutes.length === 0 && (
                  <div className="text-xs text-emerald-600">Aguardando migracoes (A2/A3)</div>
                )}
              </div>
            </div>
            <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
              ...
              <div className="space-y-0.5">
                {externalRoutes.map((r) => (
                  <div key={r.href} className="text-xs text-gray-600 font-mono">{r.href}</div>
                ))}
              </div>
            </div>
          </div>
        ...
        <div className="card p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-base font-semibold text-gray-900">Infraestrutura Financeira — Pipeline de Ingestao</h2>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-gray-200">
                  <th className="text-left py-2 px-2 font-semibold text-gray-500 uppercase tracking-wide">Camada</th>
                  <th className="text-left py-2 px-2 font-semibold text-gray-500 uppercase tracking-wide">Impl.</th>
                  <th className="text-left py-2 px-2 font-semibold text-gray-500 uppercase tracking-wide">Validacao</th>
                  <th className="text-left py-2 px-2 font-semibold text-gray-500 uppercase tracking-wide">Producao</th>
                  <th className="text-left py-2 px-2 font-semibold text-gray-500 hidden lg:table-cell uppercase tracking-wide">Notas</th>
                </tr>
              </thead>
              <tbody>
                {INGEST_LAYER_STATUS.map((layer) => (
                  <tr key={layer.name} className="border-b border-gray-100 hover:bg-gray-50/60">
                    ...
                    <td className="py-2 px-2 text-gray-500 hidden lg:table-cell max-w-xs text-xs">{layer.notes.slice(0, 100)}{layer.notes.length > 100 ? "..." : ""}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="card p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-base font-semibold text-gray-900">Governanca / Confiabilidade</h2>
          </div>
          ...
          <div className="mt-4 rounded-lg border border-gray-200 bg-gray-50 p-3">
            <div className="text-xs font-semibold text-gray-700 mb-1">Classificacao das camadas do pipeline financeiro</div>
        ...
        <div className="card p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-base font-semibold text-gray-900">Fila de Acoes — Consolidacao Pendente</h2>
            <span className="ml-auto text-xs text-gray-400">{ACTION_QUEUE.length} items</span>
          </div>
          <div className="space-y-3">
            {ACTION_QUEUE.map((item) => (
              <div key={item.id} className="rounded-lg border border-gray-200 bg-gray-50/60 p-4">
                <div className="flex items-start gap-3">
                  <span className="text-xs font-bold text-gray-400 font-mono mt-0.5 shrink-0">{item.id}</span>
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      <PriorityBadge p={item.priority} />
                      <span className="text-xs font-semibold text-gray-800">{item.title}</span>
                      <span className="ml-auto text-xs text-gray-400">{item.owner}</span>
                    </div>
          ...
          return (
            <div className="card p-5">
              <div className="flex items-center gap-2 mb-1">
                ...
                <h2 className="text-base font-semibold text-gray-900">Mapa de Migracao de Snapshots</h2>
                <span className="ml-auto text-xs text-gray-400">{migration.totalSources} fontes · {migration.totalConsumers} paginas consumidoras</span>
              </div>
              ...
              <div className="space-y-3">
                {SNAPSHOT_REGISTRY.map((src) => (
                  <div key={src.file} className="rounded-lg border border-amber-100 bg-amber-50/40 p-3">
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      <code className="text-xs font-mono font-semibold text-amber-900 bg-amber-100 px-1.5 py-0.5 rounded">{src.file}</code>
                      ...
                      <span className="text-xs text-gray-500">{src.period}</span>
                    </div>
                    <p className="text-xs text-gray-600 mb-1">{src.scope}</p>
