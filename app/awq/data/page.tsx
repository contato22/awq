...
const DATA_SOURCES: DataSourceEntry[] = [
  ...
  { name: "financial-data-bridge.ts",    path: "lib/financial-data-bridge.ts",          type: "adapter",         scope: "Financeiro",           description: "Adaptador pipeline→snapshot com fallback. Implementado mas não wired em nenhuma página.",                          isConsumed: false, isInternal: true,  confidence: "nao-verificavel", pagesUsing: 0  },
];
...
const ACTION_QUEUE: ActionItem[] = [
  ...
  { id: "A3", priority: "alto",    title: "Migrar 8 páginas AWQ de snapshot → pipeline",     description: "awq/financial, awq/cashflow, awq/kpis, awq/budget, awq/forecast, awq/allocations, awq/portfolio, awq/risk — ainda leem awq-group-data.ts.",                               owner: "Engenharia",  impact: "Elimina snapshot como fonte primária para a holding." },
  { id: "A4", priority: "medio",   title: "Ativar financial-data-bridge.ts nos dashboards",  description: "lib/financial-data-bridge.ts implementado mas sem consumidor. Estratégia de fallback (pipeline → snapshot) nunca wired.",                                                    owner: "Engenharia",  impact: "Transição suave de snapshot para real sem breaking change." },
  { id: "A5", priority: "medio",   title: "Enforçar RBAC no middleware",                     description: "auth-users.ts define roles (owner, admin, analyst, cs-ops) mas ROLE_ALLOWED_PREFIXES está aberto para todos. Sem restrição real.",                                           owner: "Segurança",   impact: "cs-ops e analyst sem acesso a dados AWQ Group." },
  { id: "A6", priority: "baixo",   title: "Atualizar dataSource no platform-registry",       description: "Após migrações, campos dataSource ainda apontam para snapshots em vez de financial-query.",                                                                                    owner: "Engenharia",  impact: "Rastreabilidade correta nesta página de management." },
  { id: "A7", priority: "baixo",   title: "Remover código órfão",                            description: "lib/data.ts:customers[] array vazio (dead code). public/data/jacqes-kpis.json desalinhado e não consumido.",                                                                  owner: "Engenharia",  impact: "Reduz confusão sobre fontes de dados." },
];
