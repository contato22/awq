      ...
      <div className="px-8 py-6 space-y-6">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <div className="xl:col-span-2 card-elevated p-5">
            <h2 className="text-sm font-bold text-slate-800 tracking-tight mb-4">Receita Mensal — Realizado + Forecast 2026</h2>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-slate-800 text-white">
                    <th className="text-left  py-2 px-3 text-xs font-bold text-white">Mês</th>
                    <th className="text-right py-2 px-3 text-xs font-bold text-white">Base</th>
                    <th className="text-right py-2 px-3 text-xs font-bold text-white">Bull</th>
                    <th className="text-right py-2 px-3 text-xs font-bold text-white">Bear</th>
                ...
                <tbody>
                  {revenueForecasts.map((row) => {
                    ...
                    return (
                      <tr key={row.month} className={`border-b border-gray-100 odd:bg-white even:bg-gray-50 hover:bg-gray-100/60 transition-colors ${!isActual ? "opacity-75" : ""}`}>
                        ...
                        <td className="py-2.5 px-3 text-right text-xs font-bold text-slate-800">
                          {isActual ? fmtR(row.actual!) : <span className="text-gray-500">—</span>}
                        </td>
                        <td className="py-2.5 px-3">
                          {isActual
                            ? <span className="badge badge-green border border-emerald-200">Realizado</span>
                            : <span className="badge border border-gray-200">Forecast</span>}
                        </td>
                      </tr>
                    );
                  })}
                ...
                <tfoot>
                  <tr className="border-t border-gray-300">
                    <td className="py-2.5 px-3 text-xs font-bold text-gray-500">2026 TOTAL</td>
                    <td className="py-2.5 px-3 text-right text-xs font-bold text-slate-800">{fmtR(fullYearBase)}</td>
          ...
          <div className="space-y-4">
            <div className="card-elevated p-5">
              <h2 className="text-sm font-bold text-slate-800 tracking-tight mb-4">Forecast Accuracy</h2>
                  ...
                  return (
                    <div key={row.month}>
                      ...
                      <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                        <div className="h-full bg-gradient-to-r from-slate-600 to-slate-400 rounded-full" style={{ width: `${acc}%` }} />
                      </div>
                      <div className="text-[10px] text-gray-500 text-right mt-0.5">{acc.toFixed(1)}% accuracy</div>
                    </div>
                  );
                })}
            ...
            <div className="card-elevated p-5">
              <h2 className="text-sm font-bold text-slate-800 tracking-tight mb-4">Forecast por BU — Full Year 2026</h2>
              <div className="space-y-3">
                {buForecasts.map((bu) => (
                  <div key={bu.bu} className="rounded-lg border border-gray-200 p-3">
                    ...
                    <div className="grid grid-cols-3 gap-1 text-center">
                      <div>
                        ...
                        <div className="text-[9px] text-gray-500">Bull</div>
                      </div>
                      <div>
                        <div className="text-xs font-bold text-slate-800">{fmtR(bu.fullYearBase)}</div>
                        <div className="text-[9px] text-gray-500">Base</div>
                      </div>
                      <div>
                        <div className="text-xs font-bold text-red-600">{fmtR(bu.fullYearBear)}</div>
                        <div className="text-[9px] text-gray-500">Bear</div>
                      </div>
                    </div>
                  </div>
                ))}
