      ...
      <div className="px-8 py-6 space-y-6">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <div className="xl:col-span-2 card p-5">
            <h2 className="text-sm font-semibold text-white mb-4">Receita Mensal — Realizado + Forecast 2026</h2>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-800">
                    ...
                    <th className="text-right py-2 px-3 text-xs font-semibold text-emerald-400">Bull</th>
                    <th className="text-right py-2 px-3 text-xs font-semibold text-red-400">Bear</th>
                    <th className="text-right py-2 px-3 text-xs font-semibold text-white">Realizado</th>
                ...
                <tbody>
                  {revenueForecasts.map((row) => {
                    ...
                    return (
                      <tr key={row.month} className={`border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors ${!isActual ? "opacity-75" : ""}`}>
                        <td className="py-2.5 px-3 text-xs font-medium text-gray-300">{row.month}</td>
                        <td className="py-2.5 px-3 text-right text-xs text-gray-400">{fmtR(row.base)}</td>
                        <td className="py-2.5 px-3 text-right text-xs text-emerald-400">{fmtR(row.bull)}</td>
                        <td className="py-2.5 px-3 text-right text-xs text-red-400">{fmtR(row.bear)}</td>
                        <td className="py-2.5 px-3 text-right text-xs font-bold text-white">
                          {isActual ? fmtR(row.actual!) : <span className="text-gray-600">—</span>}
                        </td>
                ...
                <tfoot>
                  <tr className="border-t border-gray-700">
                    ...
                    <td className="py-2.5 px-3 text-right text-xs font-bold text-white">{fmtR(fullYearBase)}</td>
                    <td className="py-2.5 px-3 text-right text-xs font-bold text-emerald-400">{fmtR(fullYearBull)}</td>
                    <td className="py-2.5 px-3 text-right text-xs font-bold text-red-400">{fmtR(fullYearBear)}</td>
                    <td className="py-2.5 px-3 text-right text-xs font-bold text-brand-400">{fmtR(consolidated.revenue)}</td>
                    <td />
                  </tr>
                </tfoot>
              </table>
          ...
          <div className="space-y-4">
            <div className="card p-5">
              <h2 className="text-sm font-semibold text-white mb-4">Forecast Accuracy</h2>
                ...
                {accuracyHistory.map((row) => {
                  ...
                  return (
                    <div key={row.month}>
                      ...
                      <div className="text-[10px] text-gray-600 text-right mt-0.5">{acc.toFixed(1)}% accuracy</div>
                    </div>
                  );
                })}
                <div className="pt-2 border-t border-gray-800 flex items-center justify-between">
                  <span className="text-xs text-gray-500">Média</span>
                  <span className="text-xs font-bold text-brand-400">{avgAccuracy.toFixed(1)}% accuracy</span>
                </div>
              </div>
            </div>

            <div className="card p-5">
              <h2 className="text-sm font-semibold text-white mb-4">Forecast por BU — Full Year 2026</h2>
              <div className="space-y-3">
                {buForecasts.map((bu) => (
                  <div key={bu.bu} className="rounded-lg border border-gray-800 p-3">
                    <div className="flex items-center justify-between mb-1.5">
                      <div className="flex items-center gap-2">
                        <div className={`w-2 h-2 rounded-full ${bu.color}`} />
                        <span className={`text-xs font-semibold ${bu.accent}`}>{bu.bu}</span>
                      </div>
                      <span className="text-[10px] text-emerald-400 font-bold">+{bu.growth}% vs 2025</span>
                    </div>
                    <div className="grid grid-cols-3 gap-1 text-center">
                      <div>
                        <div className="text-xs font-bold text-emerald-400">{fmtR(bu.fullYearBull)}</div>
                        <div className="text-[9px] text-gray-600">Bull</div>
                      </div>
                      <div>
                        <div className="text-xs font-bold text-white">{fmtR(bu.fullYearBase)}</div>
                        <div className="text-[9px] text-gray-600">Base</div>
                      </div>
                      <div>
                        <div className="text-xs font-bold text-red-400">{fmtR(bu.fullYearBear)}</div>
                        <div className="text-[9px] text-gray-600">Bear</div>
                      </div>
                    </div>
                  </div>
                ))}
