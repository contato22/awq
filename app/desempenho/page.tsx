    ...
    <>
      ...
      <div className="px-8 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
          {[
            { label: "Receita Anual", value: fmtCurrency(totalRevenue), icon: DollarSign, color: "text-brand-600", delta: `+${revGrowth}%` },
            { label: "Margem Atual", value: `${marginPct}%`, icon: TrendingUp, color: "text-emerald-600", delta: "+4.3pp" },
            { label: "Produtos Ativos", value: String(topProducts.length), icon: BarChart3, color: "text-violet-700", delta: null },
            { label: "Regiões", value: String(regionData.length), icon: Users, color: "text-amber-700", delta: null },
          ].map((kpi) => {
            const Icon = kpi.icon;
            return (
        ...
        <div className="card-elevated p-5">
          <h2 className="text-sm font-bold text-slate-800 mb-4">Tendência de Receita — 12 Meses</h2>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          {/* Products */}
          <div className="card-elevated p-5">
            <h2 className="text-sm font-bold text-slate-800 mb-4">Desempenho por Produto</h2>
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-800">
                  <th className="text-left py-2 px-3 text-xs font-semibold text-white">Produto</th>
                  <th className="text-right py-2 px-3 text-xs font-semibold text-white">Receita</th>
                  <th className="text-right py-2 px-3 text-xs font-semibold text-white">Crescimento</th>
                  <th className="text-left py-2 px-3 text-xs font-semibold text-white">Status</th>
                </tr>
              </thead>
              <tbody>
                {topProducts.map((p, idx) => (
                  <tr key={p.id} className={`border-b border-gray-100 hover:bg-gray-50 transition-colors ${idx % 2 === 1 ? "even:bg-gray-50/60" : ""}`}>
                    <td className="py-2.5 px-3">
                      <div className="text-xs font-medium text-gray-800">{p.name}</div>
                      <div className="text-[10px] text-gray-500">{p.category}</div>
                    </td>
                    <td className="py-2.5 px-3 text-right text-xs font-bold text-emerald-600">{fmtCurrency(p.revenue)}</td>
          ...
          {/* Regions */}
          <div className="card-elevated p-5">
            <h2 className="text-sm font-bold text-slate-800 mb-4">Desempenho Regional</h2>
                  ...
                  <div key={r.region}>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <div className="flex items-center gap-3 text-[11px]">
                        ...
                        <span className="font-bold text-emerald-600">{fmtCurrency(r.revenue)}</span>
                      </div>
                    </div>
