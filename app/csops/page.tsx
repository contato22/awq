    ...
    <>
      ...
      <div className="px-8 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
          {[
            { label: "Health Score Médio", value: String(avgHealth), icon: HeartPulse, color: "text-brand-600", bg: "bg-brand-50" },
            { label: "NPS Atual", value: String(lastNps), icon: TrendingUp, color: "text-emerald-600", bg: "bg-emerald-50" },
            { label: "Contas Ativas", value: String(active.length), icon: Users, color: "text-violet-700", bg: "bg-violet-50" },
            { label: "Em Risco", value: String(atRisk), icon: AlertTriangle, color: "text-amber-700", bg: "bg-amber-50" },
          ].map((kpi) => {
            const Icon = kpi.icon;
            return (
        ...
        <div className="card p-5">
          <h2 className="text-sm font-semibold text-gray-900 mb-4">Health Score por Conta</h2>
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200">
                ...
                <th className="text-left py-2 px-3 text-xs font-semibold text-gray-500">Tendência</th>
                <th className="text-right py-2 px-3 text-xs font-semibold text-gray-500">NPS</th>
                <th className="text-right py-2 px-3 text-xs font-semibold text-gray-500">Tickets</th>
            ...
            <tbody>
              {healthScores.map((h) => {
                ...
                return (
                  <tr key={h.account} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="py-2.5 px-3">
                      ...
                      {h.trend === "stable" && <span className="text-[10px] text-gray-400">—</span>}
                    </td>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          {/* CS metrics trend */}
          <div className="card p-5">
            <h2 className="text-sm font-semibold text-gray-900 mb-4">Evolução CS — Últimos 3 Meses</h2>
                ...
                <div key={m.month} className="p-3 rounded-xl bg-gray-50 border border-gray-200">
                  ...
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs font-bold text-gray-900">{m.nps}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] text-gray-400">CSAT</span>
                      <span className="text-xs font-bold text-gray-900">{m.csat}</span>
                    </div>
                    <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] text-gray-400">Tempo Res.</span>
          ...
          {/* Playbooks */}
          <div className="card p-5">
            <h2 className="text-sm font-semibold text-gray-900 mb-4">Playbooks Ativos</h2>
              ...
              {playbooks.map((p) => (
                <div key={p.name} className="p-3 rounded-xl bg-gray-50 border border-gray-200">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-[10px] text-gray-400">{p.accounts} conta{p.accounts !== 1 ? "s" : ""}</span>
                  </div>
