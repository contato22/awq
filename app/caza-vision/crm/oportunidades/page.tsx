...
import { Target, Plus, X, Database, CloudOff, HardDrive, Pencil, Trash2 } from "lucide-react";
...
const PROB_MAP: Record<string, number> = {
  ...
  "Proposta Enviada": 60, "Negociação": 75, "Fechado Ganho": 100, "Fechado Perdido": 0,
};
...
const EMPTY_FORM = {
  ...
  prazo_estimado: "", proxima_acao: "", risco: "Baixo" as string, observacoes: "",
};
...
function OppFormFields({ values, onChange }: { values: OppForm; onChange: (k: string, v: string) => void }) {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mt-3">
      ...
      <div>
        ...
        <select value={values.stage} onChange={(e) => onChange("stage", e.target.value)} className={cls}>
          {CAZA_PIPELINE_STAGES.map((s) => <option key={s}>{s}</option>)}
        </select>
      </div>
      <div>
        <label className={lbl}>Risco</label>
        <select value={values.risco} onChange={(e) => onChange("risco", e.target.value)} className={cls}>
...
export default function CazaCrmOportunidades() {
  ...
  async function handleSave() {
    if (!String(form.nome_oportunidade).trim()) { setError("Nome é obrigatório."); return; }
    if (IS_STATIC) {
      const newOpp: CazaCrmOpportunity = {
        id: lsLocalId("CV-OPP"),
        lead_id: null,
        nome_oportunidade: String(form.nome_oportunidade).trim(),
        empresa: String(form.empresa).trim(),
        tipo_servico: form.tipo_servico,
        valor_estimado: Number(form.valor_estimado) || 0,
        stage: form.stage,
        probabilidade: Number(form.probabilidade) || (PROB_MAP[form.stage] ?? 10),
      ...
      setOpps(updated); setSource("local"); setShowForm(false); setForm({ ...EMPTY_FORM });
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setOpps((prev) => [opp, ...prev]); setSource("internal");
  ...
  async function handleUpdate() {
    if (!editingId) return;
    if (IS_STATIC) {
      const updated = opps.map((o) => o.id === editingId ? {
        ...o, ...editForm,
      ...
      lsSet(LS_KEY, updated); setOpps(updated); setEditingId(null);
      return;
    }
    setSaving(true); setError(null);
    try {
  ...
  async function handleDelete(id: string) {
    if (IS_STATIC) {
      ...
      lsSet(LS_KEY, updated); setOpps(updated); setDeletingId(null);
    ...
    <>
      ...
      <div className="page-container">
        ...
        <div className="card p-5">
          ...
          {visible.length === 0 ? (
            <EmptyState compact title="Sem oportunidades" description="Crie a primeira oportunidade." />
          ) : (
