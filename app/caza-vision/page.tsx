...
const kpiColorMap: Record<string, { text: string; bg: string }> = {
  emerald: { text: "text-emerald-600", bg: "bg-slate-100" },
  brand:   { text: "text-brand-600",   bg: "bg-slate-100"   },
  violet:  { text: "text-violet-700",  bg: "bg-slate-100"  },
  amber:   { text: "text-amber-700",   bg: "bg-slate-100"   },
};

// ─── Pipeline Summary ─────────────────────────────────────────────────────────

const pipeline = [
  { stage: "Briefings",         count: 12, color: "bg-gradient-to-r from-slate-700 to-slate-500"   },
  { stage: "Em Proposta",       count:  8, color: "bg-gradient-to-r from-slate-700 to-slate-500"   },
  { stage: "Aprovados",         count: 23, color: "bg-gradient-to-r from-slate-700 to-slate-500"  },
  { stage: "Em Pós-Produção",   count:  6, color: "bg-gradient-to-r from-slate-700 to-slate-500" },
  { stage: "Entregues",         count: 34, color: "bg-gradient-to-r from-slate-700 to-slate-500"},
];
      ...
      <div className="px-8 py-6 space-y-6">
          ...
          {cazaKpis.map((kpi) => {
            ...
            const displayValue =
              kpi.unit === "currency"
                ...
                : fmtNumber(kpi.value);
            return (
              <div key={kpi.id} className="card-elevated p-5 flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1 mt-1">
                    ...
                    <span className="text-[10px] text-gray-500">vs período anterior</span>
                  </div>
                </div>
              </div>
            );
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
          ...
          <div className="card-elevated p-5">
            <h2 className="section-title mb-4">Pipeline de Projetos</h2>
              ...
              {pipeline.map((s) => {
                ...
                return (
                  <div key={s.stage} className="flex items-center gap-3">
                    <div className="w-32 text-xs text-gray-500 text-right shrink-0">{s.stage}</div>
                    <div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden">
                      <div className={`h-full rounded-full ${s.color}`} style={{ width: `${widthPct}%` }} />
                    </div>
                    <div className="w-8 text-xs font-semibold text-slate-800 text-right shrink-0">{s.count}</div>
                  </div>
                );
              })}
            </div>
            ...
            <div className="mt-5 pt-4 border-t border-gray-200 grid grid-cols-3 gap-2">
              {projectTypeRevenue.slice(0, 3).map((pt) => (
                <div key={pt.type} className="text-center">
                  <div className="text-sm font-bold text-slate-800">{pt.projetos}</div>
                  <div className="text-[10px] text-gray-500 mt-0.5">{pt.type.split(" ")[0]}</div>
                  <div className="text-[10px] text-emerald-600 font-bold">{fmtCurrency(pt.receita)}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Alerts */}
          <div className="card-elevated p-5">
            <h2 className="section-title mb-4">Alertas & Destaques</h2>
            <div className="space-y-3">
              {cazaAlerts.map((alert) => {
                const Icon = alertIcon[alert.type] ?? Info;
                const color = alertColor[alert.type] ?? "text-gray-500";
                return (
        ...
        <div className="card-elevated p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="section-title">Receita — Últimos 3 Meses</h2>
            ...
            {cazaRevenueData.slice(-3).map((m) => {
              ...
              return (
                <div key={m.month} className="p-4 rounded-xl bg-gray-100 border border-gray-300/50">
                  ...
                  <div className="text-lg font-bold text-emerald-600">{fmtCurrency(m.receita)}</div>
                  <div className="text-[11px] text-emerald-600 font-bold mt-1">Receita</div>
                  ...
                  <div className="text-[10px] text-gray-500 mt-1">Orç: {fmtCurrency(m.orcamento)}</div>
                </div>
              );
            })}
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-800">
                  ...
                  <th className="text-left py-2 px-3 text-xs font-bold text-white">Diretor</th>
                  <th className="text-left py-2 px-3 text-xs font-bold text-white">Prazo</th>
              ...
              <tbody>
                {projetos.slice(0, 6).map((p) => {
                  ...
                  return (
                    <tr key={p.id} className="border-b border-gray-100 even:bg-gray-50 hover:bg-gray-100 transition-colors">
                      <td className="py-2.5 px-3">
                        <div className="text-slate-800 font-medium text-xs truncate max-w-[180px]">
                          {p.titulo}
                        </div>
                      ...
                      <td className="py-2.5 px-3 text-xs text-gray-500">{p.diretor}</td>
                      <td className="py-2.5 px-3 text-xs text-gray-500">{p.prazo}</td>
                      <td className="py-2.5 px-3">
