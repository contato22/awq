...
import { getDREByMonth, initAllAPARTables, type BuCode } from "@/lib/ap-ar-db";
...
export async function GET(req: NextRequest) {
  const denied = await apiGuard(req, "view", "financeiro", "EPM DRE");
  ...
  try {
    ...
    return ok(await getDREByMonth(bu_code ? { bu_code } : undefined));
