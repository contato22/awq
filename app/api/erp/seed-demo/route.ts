...
export async function POST(req: NextRequest) {
  ...
  const purchases = [
    ...
    { order_number: "DEMO-PC-005", supplier_name: "Software House",            bu_code: "JACQES", order_date: daysAgo(2),  expected_date: null,        total_amount:   199.00, status: "Rascunho", notes: "Licença anual — demo" },
  ];
  ...
  const sales = [
    ...
    { order_number: "DEMO-SO-005", customer_name: "Tati Simões Produções",     bu_code: "JACQES", order_date: daysAgo(5),  delivery_date: null,        total_amount:  1790.00, status: "Em Processamento", notes: "Em andamento — demo" },
  ];

  // ── Warehouses (vazia em prod hoje) ────────────────────────────────────────
  // Sem coluna `address` — não existe no schema (confirmado empiricamente).
  const warehouses = [
    { name: "DEMO-Matriz SP",         bu_code: "AWQ"    },
    { name: "DEMO-Estúdio Pinheiros", bu_code: "JACQES" },
  ];
  ...
  try {
    // Sem coluna `amount` — não existe no schema (confirmado empiricamente).
    const movements = [
      { movement_date: daysAgo(40), type: "Entrada",       document_ref: "DEMO-MV-001", item_name: "Material escritório (DEMO)", quantity: 50,  status: "Concluída", notes: "Recebimento DEMO-PC-001" },
      { movement_date: daysAgo(25), type: "Entrada",       document_ref: "DEMO-MV-002", item_name: "Notebooks Dell (DEMO)",       quantity: 2,   status: "Concluída", notes: "Recebimento DEMO-PC-002" },
      { movement_date: daysAgo(20), type: "Transferência", document_ref: "DEMO-MV-003", item_name: "Material escritório (DEMO)", quantity: 10,  status: "Concluída", notes: "Matriz → Estúdio (demo)" },
    ];
