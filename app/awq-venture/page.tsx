...
type ReceitaMes = { mes: string; total: number | null; om: number | null; seguro: number | null; meta: number | null };
const receitaAnual2026: ReceitaMes[] = [];
...
export default function PoCPage() {
  ...
  const totalFechados = venture?.totalFechado ?? 0;
  const meta          = 120000;
  const om            = venture?.byCategoria["O&M"] ?? 0;
  const seguro        = venture?.byCategoria["Seguro"] ?? 0;
  const faturamento   = Object.values(venture?.byCategoria ?? {}).reduce((s, v) => s + v, 0);
  const canais        = (venture?.byCanal ?? []).map((c, i) => ({ ...c, color: CANAL_COLORS[i % CANAL_COLORS.length] }));
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
          ...
          <div className="grid grid-cols-3 gap-2 mb-4">
            {[
              { label: "Ano 1 (Atual)", pct: totalFechados > 0 ? Math.round((totalFechados / meta) * 100) : 0, valor: totalFechados, meta: meta },
              { label: "Ano 2",         pct: 0,   valor: 0, meta: meta },
              { label: "Ano 3",         pct: 0,   valor: 0, meta: meta },
            ].map((a) => (
        ...
        <div className="col-span-1 col-start-3 bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
          ...
          <div className="grid grid-cols-3 gap-2 mb-4 text-center">
            <div className="bg-gray-50 rounded-xl p-3">
              ...
              <div className="text-xl font-bold text-gray-900">{canais.length}</div>
            </div>
            <div className="bg-gray-50 rounded-xl p-3">
              <div className="text-xs text-gray-500 mb-1">Leads</div>
              <div className="text-xl font-bold text-gray-900">{canais.reduce((s, c) => s + c.leads, 0)}</div>
            </div>
            <div className="bg-gray-50 rounded-xl p-3">
              <div className="text-xs text-gray-500 mb-1">Valor Total</div>
              <div className="text-sm font-bold text-gray-900">R$ {fmtBRL(canais.reduce((s, c) => s + c.valor, 0))}</div>
            </div>
          </div>
          ...
          {canais.length > 0 && (
            <>
              ...
              {(() => {
                const top = canais.length > 0 ? canais.reduce((a, b) => (b.leads > a.leads ? b : a)) : null;
                return top ? (
                  <div className="mt-4 pt-3 border-t border-gray-100 text-[11px] text-gray-500">
                    Principal canal: <span className="font-semibold text-gray-700">{top.canal}</span> com {top.leads} leads ({top.pct}%)
                  </div>
                ) : null;
              })()}
            </>
