...
export default function DealSharePage({ params }: { params: { id: string } }) {
  ...
  function handleSubmitInteraction() {
    ...
    setSubmitDone(true);
    if (confirmStatus === "aguardando") setConfirmStatus("em_analise");
    if (newInteractionType === "contraproposta" && confirmStatus !== "confirmado") setConfirmStatus("negociando");
      ...
      <div className="max-w-3xl mx-auto px-6 py-8 space-y-5">
        ...
        <div className="flex items-start gap-2 p-3 rounded-xl bg-blue-50 border border-blue-200 text-xs text-blue-800">
          ...
          <span>
            ...
            Use esta página para visualizar a proposta, enviar comentários, sugestões, contrapropostas ou confirmar interesse.
          </span>
        </div>
        ...
        <div className="bg-gradient-to-b from-amber-50 to-white border border-amber-100 rounded-2xl p-6 text-center space-y-2">
          ...
          <div className="text-sm text-gray-500">{deal.operationType} · {deal.valuationRange}</div>
        ...
        <Section title="4. Proposta Econômica">
          ...
          <div className="mt-2 text-[10px] text-gray-400 bg-gray-50 rounded-lg p-2">
            {f.revenueConfidence === "probable"  && "⚠ Valores baseados em dados prováveis, não auditados externamente."}
            {f.revenueConfidence === "estimated" && "⚠ Valores são estimativas preliminares sem base documental formal."}
            {f.revenueConfidence === "confirmed" && "✓ Valores confirmados por auditoria."}
          </div>
        </Section>
        ...
        {p.stages.length > 0 && (
          <Section title="5. Cronograma">
            ...
            {p.timeline && <p className="text-xs text-gray-500 mt-3 bg-amber-50 rounded-lg px-3 py-2">{p.timeline}</p>}
          </Section>
        )}
            ...
            {confirmStatus !== "confirmado" && confirmStatus !== "rejeitado" ? (
              ...
              <div className={`text-center py-6 rounded-xl ${confirmStatus === "confirmado" ? "bg-emerald-50" : "bg-red-50"}`}>
                <div className={`text-sm font-bold ${confirmStatus === "confirmado" ? "text-emerald-700" : "text-red-700"}`}>
                  {confirmStatus === "confirmado" ? "Proposta confirmada com sucesso." : "Proposta rejeitada."}
                </div>
