// ─── /awq-venture/assets — AWQ Venture BU Asset Subledger ────────────────────
//
// DATA SOURCE: lib/asset-data.ts (AWQ Venture subledger) via lib/asset-query.ts
// SCOPE: All assets owned/custodied by AWQ Venture Investimentos. Q1 2026 snapshot.
// OWNERSHIP: This data is originated and maintained by AWQ Venture.
//            The AWQ Holding receives a read-only consolidated snapshot.
//            This page is the AWQ Venture-level detailed view.
...
function SnapshotBanner() {
  return (
    <div className="flex items-start gap-3 p-3 rounded-lg bg-amber-50 border border-amber-200 text-xs text-amber-700">
      ...
      <span>
        <strong>Snapshot Patrimonial Q1 2026 — AWQ Venture</strong> — dados do subledger de ativos da
        AWQ Venture Investimentos (lib/asset-data.ts). Este é o registro originado pelo BU.
    ...
    <div className="card p-5">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-sm font-semibold text-gray-900 flex items-center gap-2">
          ...
          Subledger de Ativos — AWQ Venture
        </h2>
        <span className="text-[10px] text-gray-400">{assets.length} ativo(s)</span>
      </div>
      {assets.length === 0 ? (
        <p className="text-xs text-gray-400 py-6 text-center">
          Nenhum ativo registrado no subledger AWQ Venture para este período.
        </p>
      ) : (
    ...
    return (
      <div className="card p-4 border-l-4 border-emerald-400 flex items-center gap-3">
        ...
        <div className="text-sm text-gray-700">
          <span className="font-semibold">Nenhum alerta de governança</span> para AWQ Venture neste período.
        </div>
      </div>
    );
  }
    ...
    <div className="card p-5 border-l-4 border-amber-400">
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-sm font-semibold text-gray-900 flex items-center gap-2">
          ...
          Alertas de Governança — AWQ Venture ({alerts.length})
        </h2>
        <Link
...
export default async function AwqVentureAssetsPage() {
  const view = await buildBuAssetView("venture");

  return (
    <>
      <Header
        title="Ativos — AWQ Venture"
        subtitle={`Subledger Patrimonial · ${view.snapshot.period} · AWQ Venture Investimentos`}
      />
      <div className="page-container">
        ...
        <div className="flex items-start gap-2 p-3 rounded-lg bg-amber-50 border border-amber-200 text-xs text-amber-700">
          ...
          <span>
            <strong>Dados originados pelo AWQ Venture.</strong> Este subledger é de propriedade e responsabilidade
            do BU AWQ Venture Investimentos. A AWQ Holding recebe uma visão consolidada de leitura — não pode
