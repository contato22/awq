    ...
    <>
      ...
      <div className="px-8 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
          {[
            { label: "Compromisso Total", value: fmtR(totalCommitment), icon: DollarSign, color: "text-emerald-600", bg: "bg-slate-100" },
            { label: "Capital Chamado",   value: fmtR(totalCalled),     icon: TrendingUp, color: "text-brand-600",   bg: "bg-slate-100" },
            { label: "LPs Confirmados",   value: String(confirmed.length), icon: CheckCircle2, color: "text-violet-700", bg: "bg-slate-100" },
            { label: "Em Pipeline",       value: String(lps.length - confirmed.length), icon: Clock, color: "text-amber-700", bg: "bg-slate-100" },
          ].map((kpi) => {
            const Icon = kpi.icon;
            return (
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          <div className="xl:col-span-2 card p-5">
            <h2 className="text-sm font-bold text-slate-800 mb-4">Base de LPs</h2>
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-800 text-white">
                  <th className="text-left py-2 px-3 text-xs font-bold">LP</th>
                  <th className="text-left py-2 px-3 text-xs font-bold">Tipo</th>
                  <th className="text-right py-2 px-3 text-xs font-bold">Compromisso</th>
                  <th className="text-right py-2 px-3 text-xs font-bold">Chamado</th>
                  <th className="text-left py-2 px-3 text-xs font-bold">Status</th>
                </tr>
              </thead>
              <tbody>
                {lps.map((lp) => (
                  <tr key={lp.id} className="border-b border-gray-100 even:bg-gray-50/60 hover:bg-gray-100 transition-colors">
                    ...
                    <td className="py-2.5 px-3 text-right text-xs font-bold text-emerald-600">{fmtR(lp.commitment)}</td>
          ...
          <div className="space-y-6">
            <div className="card-elevated p-5">
              <h2 className="text-sm font-bold text-slate-800 mb-4">Timeline de Captação</h2>
                ...
                {timeline.map((t, i) => (
                  <div key={i} className="flex items-start gap-3">
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${t.done ? "bg-emerald-100" : "bg-gray-100"}`}>
                      {t.done ? <CheckCircle2 size={12} className="text-emerald-600" /> : <Clock size={12} className="text-gray-500" />}
                    </div>
                    <div>
                      <div className={`text-xs font-medium ${t.done ? "text-gray-800" : "text-gray-500"}`}>{t.event}</div>
            ...
            <div className="card-elevated p-5">
              <h2 className="text-sm font-bold text-slate-800 mb-4">Documentação</h2>
