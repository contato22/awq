...
CREATE TABLE IF NOT EXISTS crm_accounts (
  ...
  renewal_date             DATE,
  epm_customer_id          UUID,                                           -- EPM sync ref (no FK — EPM lives in separate DB)
...
CREATE TABLE IF NOT EXISTS crm_opportunities (
  ...
  epm_customer_id     UUID,                                               -- EPM sync ref (no FK — EPM lives in separate DB)
  epm_ar_id           UUID,                                               -- EPM sync ref (no FK)
  created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  created_by          TEXT
);
...
INSERT INTO crm_accounts (account_code, account_name, trade_name, document_number, industry, company_size, account_type, bu, owner, health_score, churn_risk, address_city, address_state, website) VALUES
  ('ACC-001', 'XP Investimentos S.A.',       'XP Investimentos', '02.332.886/0001-04', 'finance',    '500+',    'customer', 'JACQES', 'Miguel', 88, 'low',    'São Paulo',      'SP', 'https://xpi.com.br'),
  ...
  ('ACC-004', 'Reabilicor Clínica Cardíaca',  'Reabilicor',       '12.345.678/0001-00', 'health',     '11-50',   'prospect', 'CAZA',   'Danilo', 60, 'medium', 'Rio de Janeiro', 'RJ', null),
  ('ACC-005', 'Clínica Teresópolis',          'Clínica Teresópolis','98.765.432/0001-11','health',    '11-50',   'customer', 'CAZA',   'Danilo', 72, 'low',    'Teresópolis',   'RJ', null),
  ('ACC-006', 'Carol Bertolini',              'Carol Bertolini',  '11.111.111/0001-22', 'media',      '1-10',    'customer', 'JACQES', 'Miguel', 91, 'low',    'São Paulo',      'SP', null)
