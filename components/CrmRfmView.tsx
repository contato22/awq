"use client";
// ─── Matriz RFM (Recência · Frequência · Monetário) — view compartilhada ─────
// Usada por /crm/rfm (todas as BUs, filtro) e /enrd/crm (só ENRD, sem filtro).
// Extraída de app/crm/rfm/page.tsx para não duplicar a grade 5×5 + lógica de
// segmentação entre as duas rotas.
...
function assignScores(vals: number[], higherIsBetter: boolean): number[] {
  ...
  return vals.map(v => {
    const rank = sorted.lastIndexOf(v);
    const score = Math.max(1, Math.round(1 + (rank / (n - 1)) * 4));
...
export type CrmRfmViewProps = {
  // Se definido, trava a BU (some o filtro) — usado por /enrd/crm.
  lockedBu?: BuFilter;
  title?: string;
  subtitle?: string;
};

export default function CrmRfmView({ lockedBu, title, subtitle }: CrmRfmViewProps) {
  const { lockedBU: sessionLockedBU } = useLockedBU();
  const effectiveLock = lockedBu ?? (sessionLockedBU as BuFilter | undefined);
  ...
  const [bu, setBu]                   = useState<BuFilter>(effectiveLock ?? "Todos");

  useEffect(() => { if (effectiveLock) setBu(effectiveLock); }, [effectiveLock]);
  ...
  const pageTitle = title ?? "Matriz RFM — CRM Tower";
  const pageSubtitle = subtitle ?? "Segmentação de clientes por Recência, Frequência e Valor Monetário";
  ...
  return (
    <>
      ...
      <div className="page-container">
        ...
        {/* BU Filter — some se travado (ex.: /enrd/crm) */}
        {!effectiveLock && (
        ...
        {effectiveLock && (
          <div className="flex items-center gap-2">
            <span className="px-3 py-1 rounded-full text-xs font-semibold bg-orange-100 text-orange-700">{effectiveLock}</span>
            <span className="text-xs text-gray-400">perímetro travado nesta BU</span>
          </div>
        )}
        ...
        <div className="card p-5">
          <SectionHeader
            ...
            title={`Clientes${selectedSegment ? ` — ${selectedSegment}` : ""}${!effectiveLock && bu !== "Todos" ? ` · ${bu}` : ""} (${visibleCustomers.length})`}
          />
