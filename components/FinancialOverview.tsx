    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-gray-200 bg-white overflow-hidden">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3 px-5 pt-4 pb-3 border-b border-gray-100">
          <div>
            <h3 className="text-sm font-bold text-gray-900">Fluxo de Caixa</h3>
            <p className="text-[10px] text-gray-400 mt-0.5">
              CD — Créditos &nbsp;·&nbsp; CI — Débitos &nbsp;·&nbsp; Saldo acumulado
            </p>
          </div>
          {/* Period selector */}
        ...
        <div className="grid grid-cols-3 divide-x divide-gray-100 px-0">
          {[
            { key: "cd",    label: "CD — Créditos",  value: totCD,      pos: true,  color: "text-emerald-600", dot: "bg-emerald-500" },
            { key: "ci",    label: "CI — Débitos",   value: totCI,      pos: false, color: "text-red-600",     dot: "bg-red-500"     },
            { key: "saldo", label: "Saldo líquido",  value: saldoFinal, pos: saldoFinal >= 0, color: saldoFinal >= 0 ? "text-blue-600" : "text-red-600", dot: "bg-blue-500" },
          ].map((k) => (
            <button
              key={k.key}
              onClick={() => toggle(k.key)}
              className={`px-5 py-3 text-left transition-opacity hover:bg-gray-50 ${hidden.has(k.key) ? "opacity-40" : ""}`}
            ...
            >
              <div className="flex items-center gap-1.5 mb-1">
                ...
                {k.key === "saldo" && netPct && (
                  <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${netVariation >= 0 ? "bg-emerald-50 text-emerald-600" : "bg-red-50 text-red-600"}`}>
                    {netVariation >= 0 ? "+" : ""}{netPct}%
                  </span>
                )}
              </div>
              <p className={`text-base font-bold tabular-nums ${k.color}`}>{fmtBRL(k.value)}</p>
            </button>
          ))}
        </div>
        ...
        <div className="px-3 pb-4 pt-2">
          <ResponsiveContainer width="100%" height={210}>
            <AreaChart data={flowData} margin={{ top: 8, right: 8, bottom: 0, left: 0 }}>
              ...
              {!hidden.has("cd") && (
                <Area
                  type="monotone" dataKey="cd" name="cd"
                  stroke="#10b981" strokeWidth={2}
                  fill="url(#gradCD)" dot={false}
                  activeDot={{ r: 4, fill: "#10b981", strokeWidth: 0 }}
                />
              )}
              {!hidden.has("ci") && (
                <Area
                  type="monotone" dataKey="ci" name="ci"
                  stroke="#ef4444" strokeWidth={2}
                  fill="url(#gradCI)" dot={false}
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-4 space-y-3">
          ...
          <div className="space-y-2">
            ...
            {txAccounts.map((acc) => {
              ...
              const bg = acc.entity === "ENERDY" ? "bg-violet-600" : acc.entity === "JACQES" ? "bg-emerald-600" : "bg-blue-600";
