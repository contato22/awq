"use client";

// ─── JACQES — Settings (Geral & Notificações) ─────────────────────────────────
// Espelha components/settings do /settings global, mas escopado à BU JACQES.
// Cosmético (sem persistência), igual ao /settings da Plataforma.

import { useState } from "react";
import { Settings, Bell, Check } from "lucide-react";
import { cn } from "@/lib/utils";

function SettingsSection({ icon: Icon, title, description, children }: {
  icon: React.ElementType; title: string; description: string; children: React.ReactNode;
}) {
  return (
    <div className="card p-5 lg:p-6">
      <div className="flex items-start gap-3.5 mb-5">
        <div className="w-9 h-9 rounded-lg bg-gray-50 border border-gray-200 flex items-center justify-center text-gray-400 shrink-0">
          <Icon size={16} />
        </div>
        <div>
          <div className="text-sm font-semibold text-gray-900">{title}</div>
          <div className="text-xs text-gray-500 mt-0.5 font-medium">{description}</div>
        </div>
      </div>
      <div className="space-y-4">{children}</div>
    </div>
  );
}

function Toggle({ label, description, defaultChecked = false }: {
  label: string; description?: string; defaultChecked?: boolean;
}) {
  const [checked, setChecked] = useState(defaultChecked);
  return (
    <button type="button" role="switch" aria-checked={checked} onClick={() => setChecked(!checked)}
      className="flex items-center justify-between py-2.5 w-full text-left group">
      <div className="pr-4">
        <div className="text-sm text-gray-700 font-medium group-hover:text-gray-900 transition-colors">{label}</div>
        {description && <div className="text-xs text-gray-400 mt-0.5">{description}</div>}
      </div>
      <div className={cn("relative w-10 h-[22px] rounded-full transition-colors shrink-0", checked ? "bg-brand-600" : "bg-gray-200")}>
        <div className={cn("absolute top-[3px] w-4 h-4 rounded-full bg-white shadow-sm transition-transform", checked ? "translate-x-[22px]" : "translate-x-[3px]")} />
      </div>
    </button>
  );
}

function FormField({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <label className="block text-xs font-medium text-gray-600 mb-1.5">{label}</label>
      {children}
    </div>
  );
}

const inputClass = "w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm text-gray-700 focus:outline-none focus:border-brand-400 focus:ring-2 focus:ring-brand-500/20 focus:bg-white transition-all placeholder:text-gray-400";

export default function JacqesSettingsForm() {
  const [saved, setSaved] = useState(false);
  const handleSave = () => { setSaved(true); setTimeout(() => setSaved(false), 2000); };

  return (
    <>
      <SettingsSection icon={Settings} title="Geral" description="Preferências de exibição da área JACQES">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <FormField label="Moeda Padrão">
            <select className={inputClass}><option>BRL — Real Brasileiro</option><option>USD — US Dollar</option></select>
          </FormField>
          <FormField label="Início do Ano Fiscal">
            <select className={inputClass}><option>Janeiro</option><option>Abril</option><option>Julho</option><option>Outubro</option></select>
          </FormField>
          <FormField label="Fuso Horário">
            <select className={inputClass}><option>UTC-3 — Brasília</option><option>UTC+0 — London</option></select>
          </FormField>
        </div>
      </SettingsSection>

      <SettingsSection icon={Bell} title="Notificações" description="Alertas específicos da BU JACQES">
        <div className="divide-y divide-gray-100">
          <Toggle label="Alertas de MRR/receita" defaultChecked={true} />
          <Toggle label="Avisos de cliente em risco" description="Alerta quando probabilidade de churn > 70%" defaultChecked={true} />
          <Toggle label="Digest semanal por e-mail" defaultChecked={true} />
        </div>
      </SettingsSection>

      <div className="flex justify-end">
        <button onClick={handleSave} className={cn("btn-primary flex items-center gap-2", saved && "bg-emerald-600 hover:bg-emerald-700")}>
          {saved ? (<><Check size={14} /> Salvo</>) : "Salvar Alterações"}
        </button>
      </div>
    </>
  );
}
