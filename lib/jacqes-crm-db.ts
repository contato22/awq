...
//
// FONTE CANÔNICA: Supabase Postgres. Seed data quando DB indisponível.
//
// Entidades:
...
// Usado quando Supabase DB não está disponível (GitHub Pages, dev local sem DB).
...
// ─── DB Init (no-op — tables managed in Supabase) ────────────────────────────
...
export async function listLeads(): Promise<CrmLead[]> {
  ...
  const { data } = await sb.from('jacqes_crm_leads').select('*').order('created_at', { ascending: false });
...
export async function createLead(data: Omit<CrmLead, "id">): Promise<CrmLead> {
  ...
  const { data: row, error } = await sb.from('jacqes_crm_leads').insert({
    nome: data.nome,
...
export async function listOpportunities(): Promise<CrmOpportunity[]> {
  ...
  const { data } = await sb.from('jacqes_crm_opportunities').select('*').order('created_at', { ascending: false });
...
export async function createOpportunity(data: Omit<CrmOpportunity, "id">): Promise<CrmOpportunity> {
  const sb = getSupabaseAdmin();
  if (!sb) throw new Error("DB not available");
  const { data: row, error } = await sb.from('jacqes_crm_opportunities').insert({
...
export async function listCrmClients(): Promise<CrmClient[]> {
  const sb = getSupabaseAdmin();
  if (!sb) return SEED_CLIENTS;
  const { data } = await sb.from('jacqes_crm_clients').select('*').order('nome', { ascending: true });
...
export async function createCrmClient(data: Omit<CrmClient, "id">): Promise<CrmClient> {
  const sb = getSupabaseAdmin();
  if (!sb) throw new Error("DB not available");
  const { data: row, error } = await sb.from('jacqes_crm_clients').insert({
    nome: data.nome,
    razao_social: data.razao_social,
    cnpj: data.cnpj,
    segmento: data.segmento,
    produto_ativo: data.produto_ativo,
    ticket_mensal: data.ticket_mensal,
    inicio_relacao: data.inicio_relacao,
    owner: data.owner,
    status_conta: data.status_conta,
    health_score: data.health_score,
    churn_risk: data.churn_risk,
    potencial_expansao: data.potencial_expansao,
    observacoes: data.observacoes,
  }).select().single();
  if (error) throw new Error(error.message);
  return row as CrmClient;
}
...
export async function listInteractions(): Promise<CrmInteraction[]> {
  const sb = getSupabaseAdmin();
  if (!sb) return SEED_INTERACTIONS;
  const { data } = await sb.from('jacqes_crm_interactions').select('*').order('created_at', { ascending: false });
...
export async function createInteraction(data: Omit<CrmInteraction, "id">): Promise<CrmInteraction> {
  const sb = getSupabaseAdmin();
  if (!sb) throw new Error("DB not available");
  const { data: row, error } = await sb.from('jacqes_crm_interactions').insert({
    cliente_id: data.cliente_id,
    ...
    satisfacao_percebida: data.satisfacao_percebida,
    risco_percebido: data.risco_percebido,
  }).select().single();
  if (error) throw new Error(error.message);
  return row as CrmInteraction;
}
...
export async function listTasks(): Promise<CrmTask[]> {
  const sb = getSupabaseAdmin();
  if (!sb) return SEED_TASKS;
  const { data } = await sb.from('jacqes_crm_tasks').select('*').order('created_at', { ascending: false });
...
export async function createTask(data: Omit<CrmTask, "id">): Promise<CrmTask> {
  const sb = getSupabaseAdmin();
  if (!sb) throw new Error("DB not available");
  const { data: row, error } = await sb.from('jacqes_crm_tasks').insert({
    cliente_id: data.cliente_id,
    opportunity_id: data.opportunity_id,
    lead_id: data.lead_id,
    titulo: data.titulo,
...
export async function updateTask(id: string, patch: Partial<Omit<CrmTask, "id">>): Promise<CrmTask | null> {
  ...
  if (patch.prioridade     !== undefined) p.prioridade     = patch.prioridade;
  if (patch.status         !== undefined) p.status         = patch.status;
  if (patch.responsavel    !== undefined) p.responsavel    = patch.responsavel;
  if (patch.sla_horas      !== undefined) p.sla_horas      = patch.sla_horas;
  if (patch.bloqueio       !== undefined) p.bloqueio       = patch.bloqueio;
  if (patch.retrabalho     !== undefined) p.retrabalho     = patch.retrabalho;
  if (patch.cliente_id     !== undefined) p.cliente_id     = patch.cliente_id;
  if (patch.opportunity_id !== undefined) p.opportunity_id = patch.opportunity_id;
  if (patch.lead_id        !== undefined) p.lead_id        = patch.lead_id;
  if (patch.prazo          !== undefined) p.prazo          = patch.prazo;
  if (patch.data_conclusao !== undefined) p.data_conclusao = patch.data_conclusao;
  const { data: row, error } = await sb.from('jacqes_crm_tasks').update(p).eq('id', id).select().single();
  if (error) throw new Error(error.message);
  return row as CrmTask;
}
...
export async function deleteTask(id: string): Promise<boolean> {
  const sb = getSupabaseAdmin();
  if (!sb) throw new Error("DB not available");
  const { error } = await sb.from('jacqes_crm_tasks').delete().eq('id', id);
  if (error) throw new Error(error.message);
  return true;
}
...
export async function listExpansion(): Promise<CrmExpansion[]> {
  const sb = getSupabaseAdmin();
  if (!sb) return SEED_EXPANSION;
  const { data } = await sb.from('jacqes_crm_expansion').select('*').order('created_at', { ascending: false });
  return (data ?? []) as CrmExpansion[];
}
...
export async function listHealth(): Promise<CrmHealthSnapshot[]> {
  const sb = getSupabaseAdmin();
  if (!sb) return SEED_HEALTH;
  const { data } = await sb.from('jacqes_crm_health_snapshot').select('*').order('created_at', { ascending: false });
  return (data ?? []) as CrmHealthSnapshot[];
}
...
export async function listProposals(): Promise<CrmProposal[]> {
  const sb = getSupabaseAdmin();
  if (!sb) return SEED_PROPOSALS;
  const { data } = await sb.from('jacqes_crm_proposals').select('*').order('created_at', { ascending: false });
