...
export const PLATFORM_ROUTES: PlatformRoute[] = [
  ...
  { href: "/",             label: "Visão Geral",    bu: "awq", layer: "control-tower", status: "active", dataSource: "lib/financial-query.ts (FCO real) + lib/awq-group-data.ts (risk/alloc snapshot)", inSidebar: true,  inTabNav: false },
  ...
  { href: "/novidades",   label: "Novidades",      bu: "awq", layer: "control-tower", status: "active", dataSource: "static (changelog inline — histórico de atualizações da plataforma)", inSidebar: true,  inTabNav: false },
  { href: "/kpis",        label: "KPIs",           bu: "awq", layer: "control-tower", status: "active", dataSource: "lib/financial-metric-query.ts → financial-query.ts (real) + snapshot (accrual KPIs)", inSidebar: true,  inTabNav: false },
  { href: "/risk",        label: "Risk & Alertas", bu: "awq", layer: "control-tower", status: "active", dataSource: "lib/financial-metric-query.ts → getAWQGroupKPIs() + getEntityCashMetrics() (real) + snapshot (risk signals)", inSidebar: true,  inTabNav: false },
  { href: "/portfolio",   label: "Portfolio",      bu: "awq", layer: "control-tower", status: "active", dataSource: "lib/financial-metric-query.ts → getPortfolioMetrics() (real caixa + snapshot capital)", inSidebar: true,  inTabNav: false },
  { href: "/allocations", label: "Allocations",    bu: "awq", layer: "control-tower", status: "active", dataSource: "lib/awq-group-data.ts (snapshot planejamento — aviso explícito na página)", inSidebar: true,  inTabNav: false },

  // ── AWQ Group — Financeiro Corporativo / FP&A ─────────────────────────────
  { href: "/financial", label: "Financial (DRE)", bu: "awq", layer: "corporate-fpa", status: "active", dataSource: "lib/financial-query.ts (pipeline canônico — sem snapshot)", inSidebar: true, inTabNav: false },
  { href: "/budget",    label: "Budget",          bu: "awq", layer: "corporate-fpa", status: "active", dataSource: "lib/awq-group-data.ts (snapshot accrual — aviso explícito na página)", inSidebar: true, inTabNav: false },
  { href: "/forecast",  label: "Forecast",        bu: "awq", layer: "corporate-fpa", status: "active", dataSource: "lib/awq-derived-metrics.ts → awq-group-data.ts (revenueForecasts Q1-only, forecastAccuracyHistory=[], buForecastScenarios=[]) — OPÇÃO C: estado vazio honesto. Projeções Abr-Dez e full-year removidas (sem metodologia, sem base histórica). revenueForecasts contém apenas Jan-Mar como referência snapshot. buildFinancialQuery() consultado apenas para status. AUDITADO 2026-04-15c: nenhum forecast ativo, nenhum dado real.", inSidebar: true, inTabNav: false },

  // ── AWQ Group — Financeiro Corporativo / Tesouraria ───────────────────────
  { href: "/cashflow",    label: "Cash Flow",     bu: "awq", layer: "corporate-treasury", status: "active", dataSource: "lib/financial-query.ts (pipeline canônico — sem snapshot)", inSidebar: true, inTabNav: false },
  { href: "/bank",        label: "Contas Banco",  bu: "awq", layer: "corporate-treasury", status: "active", dataSource: "localStorage (saldos locais — não persistido no servidor — dívida técnica)", inSidebar: true, inTabNav: false },
  { href: "/investments",  label: "Investimentos", bu: "awq", layer: "corporate-treasury", status: "active", dataSource: "lib/investment-query.ts (aplicacao/resgate financeiro — pipeline canônico)", inSidebar: true, inTabNav: false },
  { href: "/ap-ar",       label: "AP & AR",       bu: "awq", layer: "corporate-treasury", status: "active", dataSource: "localStorage (contas a pagar/receber — não persistido no servidor — dívida técnica)", inSidebar: true, inTabNav: false },
  { href: "/conciliacao", label: "Conciliação",   bu: "awq", layer: "corporate-treasury", status: "stub",   dataSource: "pending — conciliação bancária, conferência de extratos e lançamentos", inSidebar: true, inTabNav: false },

  // ── AWQ Group — Financeiro Corporativo / Controladoria ───────────────────
  { href: "/management",    label: "Controladoria",  bu: "awq", layer: "corporate-control", status: "active", dataSource: "lib/financial-metric-query.ts (diagnostics) + lib/snapshot-registry.ts + lib/platform-registry.ts", inSidebar: true, inTabNav: false },
  { href: "/contabilidade", label: "Contabilidade",  bu: "awq", layer: "corporate-control", status: "stub",   dataSource: "pending — plano de contas, balancete, lançamentos (requer pipeline contábil)", inSidebar: true, inTabNav: false },
  { href: "/fiscal",        label: "Fiscal",         bu: "awq", layer: "corporate-control", status: "stub",   dataSource: "pending — NF emitidas/recebidas, tributos, obrigações acessórias (requer integração fiscal)", inSidebar: true, inTabNav: false },

  // ── AWQ Group — Governança & Jurídico ─────────────────────────────────────
  { href: "/juridico",   label: "Jurídico",   bu: "awq", layer: "corporate-legal", status: "stub", dataSource: "pending — contratos, aditivos, vencimentos, procurações", inSidebar: true, inTabNav: false },
  { href: "/societario", label: "Societário", bu: "awq", layer: "corporate-legal", status: "stub", dataSource: "pending — quadro societário, participações, cap table, vesting, SCPs", inSidebar: true, inTabNav: false },
  { href: "/compliance", label: "Compliance", bu: "awq", layer: "corporate-legal", status: "stub", dataSource: "pending — políticas LGPD, confidencialidade, aceites, controles de acesso, obrigações regulatórias", inSidebar: true, inTabNav: false },

  // ── AWQ Group — Dados & Infra ─────────────────────────────────────────────
  { href: "/ingest",    label: "Ingestão",      bu: "awq", layer: "data-infra", status: "active", dataSource: "public/data/financial/ (documento-backed, server-side — fluxo canônico)", inSidebar: true, inTabNav: false },
  { href: "/data",      label: "Base de Dados", bu: "awq", layer: "data-infra", status: "active", dataSource: "lib/financial-ingest-status.ts + lib/platform-registry.ts + financial-db.ts (gestão real da camada de storage)", inSidebar: true, inTabNav: false },
  { href: "/security",  label: "Segurança",     bu: "awq", layer: "data-infra", status: "active", dataSource: "lib/security-types.ts + lib/security-access.ts + lib/security-registry.ts + lib/security-audit.ts + lib/security-guard.ts (RBAC matrix, sensitive routes/APIs, audit log in-memory)", inSidebar: true, inTabNav: false },
