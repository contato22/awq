...
// Uses supabase from lib/supabase.ts — falls back to seed arrays when client is null.
...
export async function getDeal(deal_id: string): Promise<MaDeal | null> {
  if (!supabase) return SEED_DEALS.find(d => d.deal_id === deal_id) ?? null;
...
export async function updateDeal(deal_id: string, data: Partial<MaDeal>): Promise<MaDeal> {
  ...
  if (data.deal_name !== undefined) patch.deal_name = data.deal_name;
  ...
  if (data.deal_type !== undefined) patch.deal_type = data.deal_type;
  if (data.industry !== undefined) patch.industry = data.industry;
  if (data.company_stage !== undefined) patch.company_stage = data.company_stage;
  if (data.market_score !== undefined) patch.market_score = data.market_score;
  if (data.team_score !== undefined) patch.team_score = data.team_score;
  if (data.product_score !== undefined) patch.product_score = data.product_score;
  if (data.traction_score !== undefined) patch.traction_score = data.traction_score;
  if (data.screening_decision !== undefined) patch.screening_decision = data.screening_decision;
  if (data.screening_notes !== undefined) patch.screening_notes = data.screening_notes;
  if (data.dd_status !== undefined) patch.dd_status = data.dd_status;
  if (data.dd_completion_pct !== undefined) patch.dd_completion_pct = data.dd_completion_pct;
  if (data.dd_start_date !== undefined) patch.dd_start_date = data.dd_start_date;
  if (data.dd_end_date !== undefined) patch.dd_end_date = data.dd_end_date;
  if (data.proposed_valuation !== undefined) patch.proposed_valuation = data.proposed_valuation;
  if (data.proposed_investment_amount !== undefined) patch.proposed_investment_amount = data.proposed_investment_amount;
  if (data.proposed_equity_pct !== undefined) patch.proposed_equity_pct = data.proposed_equity_pct;
  if (data.media_commitment_value !== undefined) patch.media_commitment_value = data.media_commitment_value;
  if (data.ic_decision !== undefined) patch.ic_decision = data.ic_decision;
  if (data.ic_decision_date !== undefined) patch.ic_decision_date = data.ic_decision_date;
  if (data.ic_decision_notes !== undefined) patch.ic_decision_notes = data.ic_decision_notes;
  if (data.ic_meeting_date !== undefined) patch.ic_meeting_date = data.ic_meeting_date;
  ...
  if (data.close_reason !== undefined) patch.close_reason = data.close_reason;
  if (data.close_notes !== undefined) patch.close_notes = data.close_notes;
  if (data.portco_id !== undefined) patch.portco_id = data.portco_id;
  if (data.deal_lead !== undefined) patch.deal_lead = data.deal_lead;
...
export async function getPortfolioCompany(
  ...
  if (!supabase) return SEED_PORTCOS.find(p => p.portco_id === portco_id) ?? null;
  const { data, error } = await supabase
    .from("v_ma_portfolio_dashboard")
    .select("*")
...
export async function updatePortfolioCompany(
  ...
  if (data.current_valuation !== undefined) patch.current_valuation = data.current_valuation;
  if (data.valuation_date !== undefined) patch.valuation_date = data.valuation_date;
  if (data.media_delivered_value !== undefined) patch.media_delivered_value = data.media_delivered_value;
  if (data.media_remaining_value !== undefined) patch.media_remaining_value = data.media_remaining_value;
  if (data.awq_ownership_pct !== undefined) patch.awq_ownership_pct = data.awq_ownership_pct;
  if (data.status !== undefined) patch.status = data.status;
  if (data.exit_date !== undefined) patch.exit_date = data.exit_date;
  if (data.exit_type !== undefined) patch.exit_type = data.exit_type;
  if (data.exit_valuation !== undefined) patch.exit_valuation = data.exit_valuation;
  if (data.exit_proceeds !== undefined) patch.exit_proceeds = data.exit_proceeds;
  if (data.ceo_name !== undefined) patch.ceo_name = data.ceo_name;
  if (data.ceo_email !== undefined) patch.ceo_email = data.ceo_email;
  if (data.ceo_phone !== undefined) patch.ceo_phone = data.ceo_phone;
  if (data.website !== undefined) patch.website = data.website;
  if (data.company_stage !== undefined) patch.company_stage = data.company_stage;
  if (data.board_seat !== undefined) patch.board_seat = data.board_seat;
  if (data.observer_rights !== undefined) patch.observer_rights = data.observer_rights;
  if (data.board_meeting_frequency !== undefined) patch.board_meeting_frequency = data.board_meeting_frequency;
  if (data.notes !== undefined) patch.notes = data.notes;
  const { data: row, error } = await supabase
    .from("ma_portfolio_companies")
    .update(patch)
...
export async function listDdItems(deal_id: string): Promise<MaDueDiligenceItem[]> {
  if (!supabase) return SEED_DD_ITEMS.filter(d => d.deal_id === deal_id);
  const { data, error } = await supabase
...
export async function updateDdItem(
  ...
  const patch: Record<string, unknown> = {};
  if (data.status !== undefined) patch.status = data.status;
  if (data.completion_pct !== undefined) patch.completion_pct = data.completion_pct;
  if (data.finding !== undefined) patch.finding = data.finding;
  if (data.finding_notes !== undefined) patch.finding_notes = data.finding_notes;
...
export async function listCapTable(portco_id: string): Promise<MaCapTableEntry[]> {
  if (!supabase) return SEED_CAP_TABLE.filter(c => c.portco_id === portco_id);
  const { data, error } = await supabase
    .from("ma_cap_table")
    .select("*")
    .eq("portco_id", portco_id)
...
export async function listPortcoKpis(portco_id: string): Promise<MaPortcoKpi[]> {
  if (!supabase) return SEED_KPIS.filter(k => k.portco_id === portco_id);
  const { data, error } = await supabase
    .from("ma_portco_kpis")
    .select("*")
    .eq("portco_id", portco_id)
...
export async function listBoardMeetings(portco_id: string): Promise<MaBoardMeeting[]> {
  if (!supabase) return SEED_BOARD_MEETINGS.filter(m => m.portco_id === portco_id);
  const { data, error } = await supabase
...
export async function updateBoardMeeting(
  ...
  if (!supabase) throw new Error("DB not available");
  const patch: Record<string, unknown> = {};
  if (data.meeting_date !== undefined) patch.meeting_date = data.meeting_date;
  if (data.agenda !== undefined) patch.agenda = data.agenda;
  if (data.board_deck_url !== undefined) patch.board_deck_url = data.board_deck_url;
  if (data.financial_report_url !== undefined) patch.financial_report_url = data.financial_report_url;
  if (data.minutes_url !== undefined) patch.minutes_url = data.minutes_url;
  if (data.awq_representative !== undefined) patch.awq_representative = data.awq_representative;
  if (data.resolutions !== undefined) patch.resolutions = data.resolutions;
...
export async function listMediaDeliverables(
  ...
  if (!supabase) return SEED_MEDIA_DELIVERABLES.filter(m => m.portco_id === portco_id);
  const { data, error } = await supabase
...
export async function updateMediaDeliverable(
  ...
  const patch: Record<string, unknown> = {};
  if (data.status !== undefined) patch.status = data.status;
  if (data.actual_delivery_date !== undefined) patch.actual_delivery_date = data.actual_delivery_date;
  if (data.approved_by_portco !== undefined) patch.approved_by_portco = data.approved_by_portco;
  if (data.approval_date !== undefined) patch.approval_date = data.approval_date;
  if (data.approval_notes !== undefined) patch.approval_notes = data.approval_notes;
  if (data.deliverable_url !== undefined) patch.deliverable_url = data.deliverable_url;
  if (data.agreed_value !== undefined) patch.agreed_value = data.agreed_value;
  if (data.executing_bu !== undefined) patch.executing_bu = data.executing_bu;
...
export async function listIntercompanyTransactions(filters?: {
  ...
  if (filters?.from_entity_id) query = query.eq("from_entity_id", filters.from_entity_id);
  if (filters?.to_entity_id) query = query.eq("to_entity_id", filters.to_entity_id);
  const { data, error } = await query;
  if (error) throw error;
...
export async function listSynergies(filters?: {
  ...
  let query = supabase
    ...
    .select("*")
    .order("created_at", { ascending: false });
  if (filters?.portco_id) query = query.eq("portco_id", filters.portco_id);
  if (filters?.source_bu) query = query.eq("source_bu", filters.source_bu);
  const { data, error } = await query;
  if (error) throw error;
...
export async function updateSynergy(
  ...
  const patch: Record<string, unknown> = {};
  if (data.status !== undefined) patch.status = data.status;
  if (data.realization_date !== undefined) patch.realization_date = data.realization_date;
  if (data.actual_revenue_impact !== undefined) patch.actual_revenue_impact = data.actual_revenue_impact;
  if (data.actual_cost_savings !== undefined) patch.actual_cost_savings = data.actual_cost_savings;
  if (data.estimated_revenue_impact !== undefined) patch.estimated_revenue_impact = data.estimated_revenue_impact;
  if (data.estimated_cost_savings !== undefined) patch.estimated_cost_savings = data.estimated_cost_savings;
...
export async function listIcMeetings(): Promise<MaIcMeeting[]> {
  if (!supabase) return [...SEED_IC_MEETINGS];
  const { data, error } = await supabase
    .from("ma_ic_meetings")
    .select("*")
    .order("meeting_date", { ascending: false });
