...
// Uses @supabase/supabase-js with NEXT_PUBLIC_SUPABASE_URL + PUBLISHABLE_KEY.
// No DATABASE_URL required — works via PostgREST REST API.
...
// ─── Schema Bootstrap (no-op — schema applied via SQL Editor) ─────────────────
export async function initCrmDB(): Promise<void> { return; }

// ─── Seed Data (static fallback when Supabase not configured) ─────────────────
...
export async function listAccounts(filters?: {
  search?: string; account_type?: string; owner?: string; bu?: string;
}): Promise<CrmAccount[]> {
  if (!hasSupabase) return [...SEED_ACCOUNTS];
  let q = db().from("crm_accounts").select("*");
  if (filters?.account_type) q = q.eq("account_type", filters.account_type);
  if (filters?.owner)        q = q.eq("owner", filters.owner);
  if (filters?.search) {
    q = q.or(`account_name.ilike.%${filters.search}%,trade_name.ilike.%${filters.search}%`);
  }
...
export async function getAccount(id: string): Promise<CrmAccount | null> {
  if (!hasSupabase) return SEED_ACCOUNTS.find(a => a.account_id === id) ?? null;
  const { data, error } = await db().from("crm_accounts").select("*").eq("account_id", id).single();
  if (error && error.code !== "PGRST116") throw new Error(error.message);
  return (data as unknown as CrmAccount) ?? null;
}

export async function createAccount(data: Partial<CrmAccount>): Promise<CrmAccount> {
  if (!hasSupabase) throw new Error("Supabase not configured");
  const { data: row, error } = await db().from("crm_accounts").insert({
...
export async function updateAccount(id: string, data: Partial<CrmAccount>): Promise<CrmAccount> {
  ...
  const { data: row, error } = await db().from("crm_accounts").update(patch).eq("account_id", id).select().single();
  if (error) throw new Error(error.message);
  return row as unknown as CrmAccount;
}
...
export async function listContacts(filters?: { account_id?: string; search?: string }): Promise<CrmContact[]> {
  if (!hasSupabase) return [...SEED_CONTACTS];
  let q = db().from("crm_contacts").select("*, crm_accounts(account_name)");
...
export async function getContact(id: string): Promise<CrmContact | null> {
  if (!hasSupabase) return SEED_CONTACTS.find(c => c.contact_id === id) ?? null;
  const { data, error } = await db().from("crm_contacts")
    .select("*, crm_accounts(account_name)").eq("contact_id", id).single();
  if (error && error.code !== "PGRST116") throw new Error(error.message);
  if (!data) return null;
  const r = data as Record<string, unknown>;
  return { ...r, account_name: (r.crm_accounts as Record<string, unknown> | null)?.account_name ?? null } as unknown as CrmContact;
}
...
export async function listLeads(filters?: { status?: string; bu?: string; assigned_to?: string }): Promise<CrmLead[]> {
  if (!hasSupabase) return [...SEED_LEADS];
  let q = db().from("crm_leads").select("*");
...
export async function getLead(id: string): Promise<CrmLead | null> {
  if (!hasSupabase) return SEED_LEADS.find(l => l.lead_id === id) ?? null;
  const { data, error } = await db().from("crm_leads").select("*").eq("lead_id", id).single();
  if (error && error.code !== "PGRST116") throw new Error(error.message);
...
export async function createLead(data: Partial<CrmLead>): Promise<CrmLead> {
  if (!hasSupabase) throw new Error("Supabase not configured");
  const { data: row, error } = await db().from("crm_leads").insert({
...
export async function updateLead(id: string, data: Partial<CrmLead>): Promise<CrmLead> {
  ...
  const { data: row, error } = await db().from("crm_leads").update(patch).eq("lead_id", id).select().single();
  if (error) throw new Error(error.message);
  return row as unknown as CrmLead;
}
...
export async function convertLead(leadId: string, oppData: Partial<CrmOpportunity>): Promise<CrmOpportunity> {
  ...
  const { error: leadErr } = await supabase.from("crm_leads").update({
    status: "converted",
    converted_to_opportunity_id: (opp as Record<string, unknown>).opportunity_id,
...
export async function listOpportunities(filters?: {
  ...
  if (!hasSupabase) return [...SEED_OPPORTUNITIES];
  let q = db().from("crm_opportunities").select(`
    *,
...
export async function getOpportunity(id: string): Promise<CrmOpportunity | null> {
  if (!hasSupabase) return SEED_OPPORTUNITIES.find(o => o.opportunity_id === id) ?? null;
  const { data, error } = await db().from("crm_opportunities")
...
export async function createOpportunity(data: Partial<CrmOpportunity>): Promise<CrmOpportunity> {
  if (!hasSupabase) throw new Error("Supabase not configured");
  const { data: row, error } = await db().from("crm_opportunities").insert({
...
export async function updateOpportunity(id: string, data: Partial<CrmOpportunity>): Promise<CrmOpportunity> {
  ...
  const { data: row, error } = await db().from("crm_opportunities").update(patch).eq("opportunity_id", id).select().single();
  if (error) throw new Error(error.message);
  return row as unknown as CrmOpportunity;
}
...
export async function listActivities(filters?: {
  ...
  if (!hasSupabase) return [...SEED_ACTIVITIES].sort((a, b) => b.created_at.localeCompare(a.created_at));
...
export async function createActivity(data: Partial<CrmActivity>): Promise<CrmActivity> {
  if (!hasSupabase) throw new Error("Supabase not configured");
  const { data: row, error } = await db().from("crm_activities").insert({
...
export async function getDashboardMetrics(): Promise<CrmDashboardMetrics> {
  const [leads, opps, activities] = await Promise.all([
    listLeads(), listOpportunities(), listActivities(),
  ]);
