...
export async function listAccounts(filters?: {
  ...
  if (!supabase) return SEED_ACCOUNTS;
...
export async function getAccount(id: string): Promise<CrmAccount | null> {
  ...
  const { data } = await supabase.from("crm_accounts").select("*").eq("account_id", id).single();
...
export async function listContacts(filters?: { account_id?: string; search?: string }): Promise<CrmContact[]> {
  if (!supabase) return SEED_CONTACTS;
  let q = supabase.from("crm_contacts").select("*, crm_accounts(account_name)").order("full_name");
...
export async function getContact(id: string): Promise<CrmContact | null> {
  ...
  if (!data) return null;
  return { ...data, account_name: (data.crm_accounts as Record<string, unknown> | null)?.account_name ?? null } as unknown as CrmContact;
}
...
export async function getLead(id: string): Promise<CrmLead | null> {
  ...
  const { data } = await supabase.from("crm_leads").select("*").eq("lead_id", id).single();
...
export async function convertLead(leadId: string, oppData: Partial<CrmOpportunity>): Promise<CrmOpportunity> {
  if (!supabase) {
    return {
      ...
      bu: oppData.bu ?? "JACQES", stage: "discovery", deal_value: oppData.deal_value ?? 0,
      probability: 25, expected_close_date: null, actual_close_date: null,
      lost_reason: null, lost_to_competitor: null, win_reason: null,
      owner: oppData.owner ?? "Miguel", proposal_sent_date: null,
...
export async function listOpportunities(filters?: {
  ...
  let q = supabase.from("crm_opportunities")
